...
| iwin | third_games_api | `antplay-bet-settle-rollback` | Antplay 投注 / 結算 / rollback 三段式流程 | 高 | Step 4 | 專案存在 / code-backed；下游 Antplay gameserver direct evidence 歸屬 iwin_gameserver | 否，作正式面試素材 | 下一步 Step 5 |
...
- `third_games_api oneapi-wallet-bet-result Step 5` 與 `third_games_api antplay-bet-settle-rollback Step 4` 已完成；若 Nick 要回原 iwin queue，下一步是 `iwin third_games_api antplay-bet-settle-rollback Step 5`。
...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | iwin | third_games_api | `antplay-bet-settle-rollback` | Antplay 投注 / 結算 / rollback 三段式流程 | Step 4 已完成；下一步做單條 flow claim gate，確認是否維持 interview-only | `iwin third_games_api antplay-bet-settle-rollback Step 5` |

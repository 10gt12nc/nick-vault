...
| antplay | antplay-slot-game-job | `proxy-user-data-report-projection` | 代理玩家報表 Kafka projection / Quartz summary | 高 | Step 3 / 2026-05-22 | 真實開發過 + code-backed；`#386` / `#590` / `#702` / `fix ag_report_player` direct evidence；source fetch 失敗，依本地 refs / 本地 working tree | 否，Step 3 不直接更新 05 / 08；等 Step 5 claim gate 回填 | 已完成；下一步 Step 4 |

...
- `antplay-slot-game-api` 已有 contribution consolidation、Step 1 candidate flows、Step 2 ranking，`slot-bet-settle-rollback Step 5` 與 `transfer-wallet-money-in-out Step 5` 已完成；下一步依同 project ranking 做 `request-log-rabbitmq-async Step 3`。
...
| 7 | antplay | antplay-slot-game-api | `request-log-rabbitmq-async` | Request log RabbitMQ 非同步化 | slot bet 與 transfer wallet 兩條 money flow 已 Step 5；下一步補 async audit / observability case | `antplay antplay-slot-game-api request-log-rabbitmq-async Step 3` |

...
- 若目標是差異化面試題，`*-math` Step 2 已完成；下一步做 `antplay *-math fixed-multi-bet-currency-math-core-compatibility Step 3`。
...
| antplay | `*-math` | `step2-flow-comparison` | fixedMultiBet / currency / math-core compatibility 第一順位 | 高 | Step 2 / 2026-05-20 | 真實開發過 + code-backed；依本地 refs，部分 remote 未確認最新 | 否，先作 Flow Track 排序 | 下一步 Step 3 |
...
- `*-math` Step 2 已完成；第一條代表 flow 選 `fixed-multi-bet-currency-math-core-compatibility`，下一步進 Step 3。
...
| 9 | antplay | `*-math` | `fixed-multi-bet-currency-math-core-compatibility` | fixedMultiBet / currency / math-core 相容 | 差異化高且最貼近 backend contract / money-like correctness；Step 2 已選為第一條代表 flow | `antplay *-math fixed-multi-bet-currency-math-core-compatibility Step 3` |

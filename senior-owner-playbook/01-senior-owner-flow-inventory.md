...
| 10 | ugsoft | ugsoft-connector-api | `provider-callback-bet-settle-to-mq` | UGSoft provider callback / bet-settle / MQ | 可選加強：第一條 `transfer-wallet-in-out-query` 已完成 Step 5；若 Nick 要繼續補非 iwin 廣度，下一步做第二順位 callback / MQ flow Step 3 | `ugsoft ugsoft-connector-api provider-callback-bet-settle-to-mq Step 3` |

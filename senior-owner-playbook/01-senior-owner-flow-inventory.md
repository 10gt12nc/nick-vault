...
- `antplay-slot-game-api` 已有 contribution consolidation、Step 1 candidate flows、Step 2 ranking，`slot-bet-settle-rollback Step 5` 已完成，且 `transfer-wallet-money-in-out Step 3` 已建立；下一步繼續同一條 flow 轉 Step 4 面試 case。
...
| 7 | antplay | antplay-slot-game-api | `transfer-wallet-money-in-out` | Transfer wallet 轉入 / 轉出 / 全轉出 | Step 3 已完成；下一步轉正式面試 case，補 idempotency、DB / Redis consistency、failure window 講法 | `antplay antplay-slot-game-api transfer-wallet-money-in-out Step 4` |

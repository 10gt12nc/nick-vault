...
| antplay | antplay-slot-game-api | `contribution-claim-consolidation` | 遊戲 API runtime / 下注結算 / 轉帳錢包 / RabbitMQ request log 收口 | 高 | rolling 已完成 / 2026-05-20；2026-05-21 五條代表 flow 全部 Step 5，待 refresh | 真實開發過 + code-backed；Nick / `10gt12nc` 有大量 direct commits | 是，保守補入 game runtime / betting-settlement / transfer wallet / async log / high-traffic table governance / runtime decision；仍不寫完整 RTP / math owner | 待收口；下一步 `antplay-slot-game-api contribution claim consolidation` |
...
- `antplay-slot-game-api` 已有 rolling contribution consolidation、Step 1 candidate flows、Step 2 ranking，`slot-bet-settle-rollback Step 5`、`transfer-wallet-money-in-out Step 5`、`request-log-rabbitmq-async Step 5`、`bet-record-sharding-schema-route Step 5` 與 `runtime-rtp-darkpool-player-control Step 5` 已完成；下一步做 `contribution claim consolidation` refresh。
...
| 7 | antplay | antplay-slot-game-api | `contribution claim consolidation` | 五條代表 flow Step 5 後的 project-level claim refresh | slot bet、transfer wallet、request log、bet-record sharding、runtime RTP 五條代表 flow 已 Step 5；可把 rolling consolidation refresh 成目前更完整的履歷 / 面試邊界 | `antplay antplay-slot-game-api contribution claim consolidation` |

...
| antplay | antplay-slot-game-api | `bet-record-sharding-schema-route` | Bet record / request log / transfer transaction 分表與 schema routing | 中高 | Step 5 / 2026-05-21 | 真實開發過 + code-backed；`10gt12nc` 有 #167 分表、db partition v2、`@UseSchema`、table creator 與 bet record query fix direct commits；source fetch 失敗，依本地 refs / 本地 working tree；logical-to-physical live route 待確認 | 是，回填 project-level high-traffic table governance；不單獨寫完整 sharding owner | 已完成；後續 `runtime-rtp-darkpool-player-control Step 3` 已完成 |
| antplay | antplay-slot-game-api | `runtime-rtp-darkpool-player-control` | RTP / dark pool / player control runtime decision | 中高 | Step 3 / 2026-05-21 | 真實開發過 + code-backed；`10gt12nc` 有 GameFacade runtime decision 周邊、dark pool / target RTP / player control context commits；source fetch 失敗，依本地 refs / 本地 working tree；math module / consumer / reconciliation 待確認 | 暫作面試素材初版；待 Step 5 gate 後再決定是否回填履歷 | 下一步 `runtime-rtp-darkpool-player-control Step 4` |
...
- `antplay-slot-game-api` 已有 contribution consolidation、Step 1 candidate flows、Step 2 ranking，`slot-bet-settle-rollback Step 5`、`transfer-wallet-money-in-out Step 5`、`request-log-rabbitmq-async Step 5` 與 `bet-record-sharding-schema-route Step 5` 已完成，`runtime-rtp-darkpool-player-control Step 3` 已完成；下一步做 `runtime-rtp-darkpool-player-control Step 4`。
...
| 7 | antplay | antplay-slot-game-api | `runtime-rtp-darkpool-player-control` | RTP / dark pool / player control runtime decision | slot bet、transfer wallet、request log、bet-record sharding 四條代表 flow 已 Step 5；本 flow Step 3 已補 runtime / math contract / risk-control boundary | `antplay antplay-slot-game-api runtime-rtp-darkpool-player-control Step 4` |

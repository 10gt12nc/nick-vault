...
- 若目標是差異化面試題，`*-math fixed-multi-bet-currency-math-core-compatibility` Step 5 已完成，`rtp-reel-strip-simulation-validation` Step 3 已完成；下一步做 `rtp-reel-strip-simulation-validation Step 4`。
...
| antplay | `*-math` | `rtp-reel-strip-simulation-validation` | RTP / 輪帶模擬與驗證 | 高 | Step 3 / 2026-05-20 | 真實開發過 + code-backed；`sph-math` 主樣本、`spn-math` 對照 path | 併入 `*-math` grouped bullet；Step 3 不直接改 05 / 08 | 下一步 Step 4 |
...
- `*-math fixed-multi-bet-currency-math-core-compatibility` Step 5 已完成，`rtp-reel-strip-simulation-validation` Step 3 已完成，下一步繼續同一條做 Step 4。
...
| 9 | antplay | `*-math` | `rtp-reel-strip-simulation-validation` | RTP / 輪帶模擬與驗證 | Step 3 已完成；下一條補 slot math 差異化最高的 simulation / validation 面試題材 | `antplay *-math rtp-reel-strip-simulation-validation Step 4` |

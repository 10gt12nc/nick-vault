...
| antplay | antplay-slot-admin-api | `game-api-whitelist-sync` | Game API 白名單控制面 / DB + Redis 同步 | 中高 | Step 5 / 2026-05-28 | 真實開發過 + code-backed；admin-api `#682` 白名單 control plane direct evidence，game-api `#684` runtime `WhiteIpFilter` direct evidence；fetch 失敗，依本地 refs / 本地工作樹判斷 | 不直接更新 05 / 08；可回填 project-level 後台 control plane / Game API 白名單 supporting evidence | 已完成；下一步可選 `antplay-slot-admin-api contribution claim consolidation refresh` |

...
- 若目標是最快產出 Senior Backend 履歷素材，`payment-provider-callback Step 5` 與 `withdrawal-auto-review-refund Step 5` 都已完成；下一條同 project 可做 `payment-order-provider-request Step 3`。
...
- 下一條同 project 高價值候選是 `payment-order-provider-request`。
...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | iwin | payment | `payment-order-provider-request` | 金流訂單與 provider request | payment callback / withdrawal 已 Step 5；下一條補 provider request、簽章、timeout、provider order id 與查單邊界 | `iwin payment payment-order-provider-request Step 3` |
...
| 5 | iwin | payment | `manual-order-review-repair` | 人工審核 / 補單 / 訂單修復 | 支援 payment repair boundary，可接 app_bi 人工入口 | payment 後續候選 |

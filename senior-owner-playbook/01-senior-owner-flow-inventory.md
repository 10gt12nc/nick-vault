...
- 若目標是最快產出 Senior Backend 履歷素材，依最新 KB 先檢查 Step 2 定義的本批代表 flows 是否都完成。`game_api` 目前只有 `coupon-redeem-credit-grant` 一條 flow 完整到 Step 5，不能直接做完整 project-level consolidation；下一步應回 Step 2 ranking，做第二順位 `partner-deposit-withdraw-bill Step 3`。`payment` 已完成 project-level consolidation，不需要因新規則重做。`game_job` Top 5 代表 flows 已完成到 Step 5，才可排 `game_job contribution claim consolidation`；consolidation 必須掃 code + 所有 game_job flow KB。
...
| iwin | game_api | `partner-deposit-withdraw-bill` | Partner API 上分 / 下分 / 查單 | 很高 | Step 2 candidate | 專案存在 / code-backed；Nick 貢獻待掃 | 待確認 | queue 第 1 |
...
- 下一步應回到 Step 2 ranking，做第二順位 `partner-deposit-withdraw-bill Step 3`，補足 `game_api` 本批代表 money API flow。
...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | iwin | game_api | `partner-deposit-withdraw-bill` | Partner API 上分 / 下分 / 查單 | game_api 只有 coupon 一條完整 flow，Step 2 本批代表 flows 未完成；先補第二順位 money API | `iwin game_api partner-deposit-withdraw-bill Step 3` |

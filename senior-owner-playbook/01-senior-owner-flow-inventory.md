...
- 若目標是差異化面試題，`*-math fixed-multi-bet-currency-math-core-compatibility` Step 3 已完成；下一步做 Step 4。
...
| antplay | `*-math` | `step2-flow-comparison` | fixedMultiBet / currency / math-core compatibility 第一順位 | 高 | Step 2 / 2026-05-20 | 真實開發過 + code-backed；依本地 refs，部分 remote 未確認最新 | 否，先作 Flow Track 排序 | 已收斂；Step 3 已完成 |
| antplay | `*-math` | `fixed-multi-bet-currency-math-core-compatibility` | fixedMultiBet / currency / math-core 相容 flow | 高 | Step 3 / 2026-05-20 | 真實開發過 + code-backed；`math-core`、`sdt-math`、`sfm-math`、`slc-math` 代表 path | 否，單條 flow 素材；05/08 等 Step 4/5 或 rolling package 回填 | 下一步 Step 4 |
...
- `*-math fixed-multi-bet-currency-math-core-compatibility` Step 3 已完成，下一步進 Step 4。
...
| 9 | antplay | `*-math` | `fixed-multi-bet-currency-math-core-compatibility` | fixedMultiBet / currency / math-core 相容 | 差異化高且最貼近 backend contract / money-like correctness；Step 3 已完成 | `antplay *-math fixed-multi-bet-currency-math-core-compatibility Step 4` |

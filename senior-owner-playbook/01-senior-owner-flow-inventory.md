...
- `antplay-slot-game-api` 已有 contribution consolidation、Step 1 candidate flows、Step 2 ranking，且 `slot-bet-settle-rollback Step 5` 已完成；下一步回 Step 2 Rank 2，深挖 transfer wallet money in/out。
...
| 7 | antplay | antplay-slot-game-api | `transfer-wallet-money-in-out` | Transfer wallet 轉入 / 轉出 / 全轉出 | `slot-bet-settle-rollback` 已完成 Step 5；下一條補 wallet consistency 主體 | `antplay antplay-slot-game-api transfer-wallet-money-in-out Step 3` |

...
| ugsoft | ugsoft-connector-api | `provider-callback-bet-settle-to-mq` | Provider callback / bet-settle / MQ 入庫 | 高 | Step 4 / 2026-05-27 | 真實開發過 + code-backed 混合；callback 寫 MQ、admin consumer、currency / pt_day 有 Nick direct evidence，最新 whitelist / subAgent / amount scaling 為團隊 context | 正式面試素材候選；待 Step 5 claim gate | 下一步 Step 5 |
...
| 10 | ugsoft | ugsoft-connector-api | `provider-callback-bet-settle-to-mq` | UGSoft provider callback / bet-settle / MQ | 可選加強：第一條 `transfer-wallet-in-out-query` 已完成 Step 5；第二條 callback / MQ flow Step 4 已完成；若 Nick 要繼續補非 iwin 廣度，下一步做 Step 5 claim gate | `ugsoft ugsoft-connector-api provider-callback-bet-settle-to-mq Step 5` |

...
| antplay | antplay-slot-game-api | `contribution-claim-consolidation` | 遊戲 API runtime / 下注結算 / 轉帳錢包 / RabbitMQ request log 收口 | 高 | 已完成 / 2026-05-20 | 真實開發過 + code-backed；Nick / `10gt12nc` 有大量 direct commits | 是，保守補入 game runtime / betting-settlement / transfer wallet / async log | 已收斂；Flow Track Step 1 已完成，下一步 Step 2 |
| antplay | antplay-slot-game-api | `step1-candidate-flows` | slot bet / transfer wallet / request log / sharding / RTP runtime 候選 flow 盤點 | 高 | Step 1 / 2026-05-21 | 真實開發過 + code-backed；source fetch 失敗，依本地 refs / 本地 working tree | 否，先作 Flow Track 選題；不直接更新 05 / 08 | 下一步 Step 2 |
...
- `antplay-slot-game-api` 已有 contribution consolidation 與 Step 1 candidate flows；下一步要做 Step 2，比較 `slot-bet-settle-rollback`、`transfer-wallet-money-in-out`、`request-log-rabbitmq-async`、`bet-record-sharding-schema-route`、`runtime-rtp-darkpool-player-control`，選第一條進 Step 3。
...
| 7 | antplay | antplay-slot-game-api | `slot-bet-settle-rollback` | Antplay 投注 / 結算 / rollback | Step 1 已完成；Step 2 要和 transfer wallet / request log / sharding / RTP runtime 比較後決定第一條深挖 | `antplay antplay-slot-game-api Step 2` |

...
- 若目標是最快產出 Senior Backend 履歷素材，payment Top 5 flow、`game_api coupon-redeem-credit-grant Step 5` 與 `game_job daily-game-data-summary Step 5` 都已收斂；下一步回到 `game_job third-party-record-mongo-backup Step 3`。
...
| iwin | game_job | `third-party-record-mongo-backup` | 第三方遊戲紀錄 Mongo 備份與清理 | 中高 | Step 2 | 專案存在 / code-backed；Nick 貢獻依三層 claim gate 判斷 | 否 | queue 第 1 |
...
- 同 project 下一條最值得做 `third-party-record-mongo-backup Step 3`。
...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | iwin | game_job | `third-party-record-mongo-backup` | 第三方遊戲紀錄 Mongo 備份與清理 | daily summary 已收斂；下一條補 backup / delete partial failure 與 retention policy | `iwin game_job third-party-record-mongo-backup Step 3` |

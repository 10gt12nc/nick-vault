...
- 若目標是差異化面試題，`*-math fixed-multi-bet-currency-math-core-compatibility` Step 5 已完成，`rtp-reel-strip-simulation-validation` Step 5 已完成，`buy-free-scatter-rtp3-result-contract` Step 5 已完成，`jackpot-symbol-hit-and-prize-scaling` Step 5 已完成，`special-wild-feature-state-transform` Step 5 已完成；`*-math contribution claim consolidation` refresh 也已完成。`antplay-slot-game-api slot-bet-settle-rollback Step 5` 與 `transfer-wallet-money-in-out Step 5` 已完成，`request-log-rabbitmq-async Step 4` 也已完成；下一步若繼續 antplay game-api，建議做 `request-log-rabbitmq-async Step 5`。
...
| antplay | antplay-slot-game-api | `transfer-wallet-money-in-out` | Transfer wallet 轉入 / 轉出 / 全額轉出 / 查單 | 很高 | Step 5 / 2026-05-21 | 真實開發過 + code-backed；`10gt12nc` 有分表 / schema route / transaction lookup / `updatePlayerWallet` rows check direct commits，API 初版多為他人 commits；source fetch 失敗，依本地 refs / 本地 working tree | 是，回填 project-level consolidation；不單獨寫完整 transfer wallet owner | 已完成；下一條 `request-log-rabbitmq-async Step 4` 已完成 |
| antplay | antplay-slot-game-api | `request-log-rabbitmq-async` | Request log RabbitMQ 非同步化 | 中高 | Step 4 / 2026-05-21 | 真實開發過 + code-backed；`10gt12nc` 有 #774 producer / consumer direct commits；source fetch 失敗，依本地 refs / 本地 working tree | 暫不直接更新 05 / 08；Step 5 前作 async audit / observability 面試素材 | 下一步 `request-log-rabbitmq-async Step 5` |
...
- `antplay-slot-game-api` 已有 contribution consolidation、Step 1 candidate flows、Step 2 ranking，`slot-bet-settle-rollback Step 5`、`transfer-wallet-money-in-out Step 5` 與 `request-log-rabbitmq-async Step 4` 已完成；下一步依同 project ranking 做 `request-log-rabbitmq-async Step 5`。
...
| 7 | antplay | antplay-slot-game-api | `request-log-rabbitmq-async` | Request log RabbitMQ 非同步化 | slot bet 與 transfer wallet 兩條 money flow 已 Step 5，request log Step 4 已完成；下一步做 claim gate | `antplay antplay-slot-game-api request-log-rabbitmq-async Step 5` |

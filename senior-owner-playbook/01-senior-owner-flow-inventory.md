...
| iwin | third_games_api | `gsc-seamless-withdraw-deposit-cancel` | GSC split withdraw / deposit / rollback / cancel | 中高 | Step 3 | 專案存在 / code-backed；split endpoint production status 待確認 | 否，Step 4 前只作面試素材初版 | `iwin third_games_api gsc-seamless-withdraw-deposit-cancel Step 4` |
...
- `third_games_api oneapi-wallet-bet-result Step 5`、`third_games_api antplay-bet-settle-rollback Step 5` 與 `third_games_api gsc-seamless-withdraw-deposit-cancel Step 3` 已完成；若 Nick 要回原 iwin queue，下一步是 `iwin third_games_api gsc-seamless-withdraw-deposit-cancel Step 4`。
...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | iwin | third_games_api | `gsc-seamless-withdraw-deposit-cancel` | GSC seamless withdraw / deposit / rollback / cancel | Step 3 已完成；下一步把 split endpoint 狀態機、duplicate guard 與 production 待確認邊界轉正式面試 case | `iwin third_games_api gsc-seamless-withdraw-deposit-cancel Step 4` |

...
| antplay | antplay-slot-game-api | `contribution-claim-consolidation` | 遊戲 API runtime / 下注結算 / 轉帳錢包 / RabbitMQ request log 收口 | 高 | 已完成 / 2026-05-20 | 真實開發過 + code-backed；Nick / `10gt12nc` 有大量 direct commits | 是，保守補入 game runtime / betting-settlement / transfer wallet / async log | 已收斂；`slot-bet-settle-rollback Step 3` 已完成，下一步 Step 4 |
...
| antplay | antplay-slot-game-api | `slot-bet-settle-rollback` | Slot 下注 / 開獎 / 結算 / rollback | 很高 | Step 3 / 2026-05-21 | code-backed + Nick / `10gt12nc` path-specific commits 線索；source fetch 失敗，依本地 refs / 本地 working tree | 否，Step 3 不直接更新 05 / 08；待 Step 5 claim gate | 下一步 Step 4 |
...
- `antplay-slot-game-api` 已有 contribution consolidation、Step 1 candidate flows、Step 2 ranking，且 `slot-bet-settle-rollback Step 3` 已完成；下一步要轉成 Step 4 面試 case。
...
| 7 | antplay | antplay-slot-game-api | `slot-bet-settle-rollback` | Antplay 投注 / 結算 / rollback | Step 3 已完成；下一步轉成可面試案例，補 Senior 追問與 failure scenario | `antplay antplay-slot-game-api slot-bet-settle-rollback Step 4` |

...
- 若目標是最快產出 Senior Backend 履歷素材，`payment-provider-callback Step 5`、`withdrawal-auto-review-refund Step 5`、`payment-order-provider-request Step 5`、`manual-order-review-repair Step 5` 都已完成，`payment-channel-config-selection Step 4` 也已完成；下一步做 `payment-channel-config-selection Step 5`。
...
| iwin | payment | `payment-channel-config-selection` | 支付列表 / 商戶設定選擇 | 中 | Step 4 | 專案存在 / code-backed；Nick 貢獻待確認 | 否 | `iwin payment payment-channel-config-selection Step 5` |
...
- `payment-channel-config-selection Step 4` 已完成，下一步同 flow 做 Step 5 claim gate。
...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | iwin | payment | `payment-channel-config-selection` | 支付列表 / 商戶設定選擇 | Step 4 已完成；檢查履歷 / 自傳 claim gate | `iwin payment payment-channel-config-selection Step 5` |

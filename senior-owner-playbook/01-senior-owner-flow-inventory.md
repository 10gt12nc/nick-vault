...
- 若目標是差異化面試題，`*-math fixed-multi-bet-currency-math-core-compatibility` Step 5 已完成，`rtp-reel-strip-simulation-validation` Step 5 已完成，`buy-free-scatter-rtp3-result-contract` Step 5 已完成，`jackpot-symbol-hit-and-prize-scaling` Step 5 已完成；下一步若繼續 `*-math`，回 Step 2 ranking 做 `special-wild-feature-state-transform Step 3`。
...
- `*-math fixed-multi-bet-currency-math-core-compatibility` Step 5 已完成，`rtp-reel-strip-simulation-validation` Step 5 已完成，`buy-free-scatter-rtp3-result-contract` Step 5 已完成，`jackpot-symbol-hit-and-prize-scaling` Step 5 已完成，下一步做 Rank 5 `special-wild-feature-state-transform Step 3`。
...
| 9 | antplay | `*-math` | `special-wild-feature-state-transform` | Special Wild / symbol state transform | `*-math` 前四條代表 flow 已完成 Step 5；若繼續同 project，回 Step 2 Rank 5 補 feature state transform | `antplay *-math special-wild-feature-state-transform Step 3` |

...
- `antplay-slot-game-api` 已有 contribution consolidation、Step 1 candidate flows、Step 2 ranking，`slot-bet-settle-rollback Step 5`、`transfer-wallet-money-in-out Step 5`、`request-log-rabbitmq-async Step 5` 與 `bet-record-sharding-schema-route Step 3` 已完成；下一步依同 project ranking 做 `bet-record-sharding-schema-route Step 4`。
...
| 7 | antplay | antplay-slot-game-api | `bet-record-sharding-schema-route` | Bet record / request log / transfer transaction 分表與 schema routing | Step 3 已完成；下一步把高流量資料治理 / schema route 題轉成正式面試 case | `antplay antplay-slot-game-api bet-record-sharding-schema-route Step 4` |

...
- 若目標是差異化面試題，`*-math fixed-multi-bet-currency-math-core-compatibility` Step 5 已完成，`rtp-reel-strip-simulation-validation` Step 4 已完成；下一步做 `rtp-reel-strip-simulation-validation Step 5`。
...
| antplay | `*-math` | `fixed-multi-bet-currency-math-core-compatibility` | fixedMultiBet / currency / math-core 相容 flow | 高 | Step 5 / 2026-05-20 | 真實開發過 + code-backed；`math-core`、`sdt-math`、`sfm-math`、`slc-math` 代表 path | 是，作 `*-math` grouped 履歷 bullet 強化 evidence；不單獨寫完整 owner | 已完成；下一步回 Step 2 Rank 2 |
| antplay | `*-math` | `rtp-reel-strip-simulation-validation` | RTP / 輪帶模擬與驗證 | 高 | Step 4 / 2026-05-20 | 真實開發過 + code-backed；`sph-math` 主樣本、`spn-math` 對照 path | 併入 `*-math` grouped bullet；Step 4 不直接改 05 / 08 | 下一步 Step 5 |
...
- `*-math fixed-multi-bet-currency-math-core-compatibility` Step 5 已完成，`rtp-reel-strip-simulation-validation` Step 4 已完成，下一步繼續同一條做 Step 5。
...
| 9 | antplay | `*-math` | `rtp-reel-strip-simulation-validation` | RTP / 輪帶模擬與驗證 | Step 4 已完成；下一條做 slot math 差異化最高的 simulation / validation claim gate | `antplay *-math rtp-reel-strip-simulation-validation Step 5` |

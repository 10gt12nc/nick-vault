...
- 若目標是最快產出 Senior Backend 履歷素材，`payment-provider-callback Step 5` 已完成，`withdrawal-auto-review-refund Step 3` 也已完成；下一條優先做 `withdrawal-auto-review-refund Step 4`。
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| iwin | app_bi | `point-control-admin-operation` | 單點控制 / 營運控制操作 | 中 | Step 5 | 專案存在 / Nick 貢獻待確認 | 否 | app_bi 已收斂，轉 payment |
| iwin | app_bi | `admin-config-redis-sync` | 後台設定同步 Redis | 中 | Step 5 | 專案存在 / Nick 貢獻待確認 | 否 | app_bi 已收斂，轉 payment |
| iwin | app_bi | `daily-game-record-summary` | 每日遊戲資料彙總 | 中 | Step 5 | 專案存在 / Nick 貢獻待確認 | 否 | app_bi 已收斂，轉 payment |
...
| iwin | payment | `payment-provider-callback` | 金流 provider callback | 高 | Step 5 | 專案存在 / code-backed；Nick 貢獻待確認 | 否 | 同 project 下一條已接 `withdrawal-auto-review-refund` |
| iwin | payment | `withdrawal-auto-review-refund` | 玩家提款、自動審核 / 自動出款與失敗退款 | 高 | Step 3 | 專案存在 / code-backed；Nick 貢獻待確認 | 否 | `iwin payment withdrawal-auto-review-refund Step 4` |
...
| iwin | iwin_gameserver | `center-http-deposit-withdraw` | center_http 上分 / 下分 | 高 | Step 2 | 專案存在 / code-backed；Nick 貢獻待確認 | 否 | `iwin_gameserver center-http-deposit-withdraw Step 3` |
| iwin | k3s-deploy | `gameserver-phased-rollout` | gameserver phase rollout / rollback | 中高 | Step 4 | 專案存在 / code-backed；Nick 貢獻待確認 | 否 | `iwin k3s-deploy gameserver-phased-rollout Step 5` |
...
- 這條 flow 還需要補 failure / consistency / idempotency / retry / reconciliation，才適合轉 Step 5 claim gate。
- 目前仍不更新正式履歷 / 自傳。
...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | iwin | payment | `withdrawal-auto-review-refund` | 提款自動審核 / 自動出款 / 失敗退款 | Step 3 已完成；下一步補扣分建單、MQ enqueue、provider no callback、重複退款與下游去重 evidence | `iwin payment withdrawal-auto-review-refund Step 4` |
...
| 3 | iwin | payment | `payment-order-provider-request` | 金流訂單與 provider request | 訂單狀態、provider contract、timeout / retry | payment 後續候選 |
...
| 5 | iwin | game_job | `daily-game-data-summary` | 每日遊戲資料彙總 | 已完成 Step 4，下一步做履歷 / claim gate | `game_job daily-game-data-summary Step 5` |
| 6 | iwin | iwin_gameserver | `center-http-deposit-withdraw` | center_http 上分 / 下分 | money correctness / center wallet mutation / idempotency | `iwin_gameserver center-http-deposit-withdraw Step 3` |

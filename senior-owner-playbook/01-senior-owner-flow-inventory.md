...
| iwin | third_games_api | `antplay-bet-settle-rollback` | Antplay 投注 / 結算 / rollback 三段式流程 | 高 | Step 3 | 專案存在 / code-backed；下游 Antplay gameserver direct evidence 歸屬 iwin_gameserver | 否，作面試素材初版 | 下一步 Step 4 |
...
- `third_games_api oneapi-wallet-bet-result Step 5` 與 `third_games_api antplay-bet-settle-rollback Step 3` 已完成；若 Nick 要回原 iwin queue，下一步是 `iwin third_games_api antplay-bet-settle-rollback Step 4`。
...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | iwin | third_games_api | `antplay-bet-settle-rollback` | Antplay 投注 / 結算 / rollback 三段式流程 | Step 3 已完成；下一步轉成可面試講的三段式 state transition / retry failure window case | `iwin third_games_api antplay-bet-settle-rollback Step 4` |

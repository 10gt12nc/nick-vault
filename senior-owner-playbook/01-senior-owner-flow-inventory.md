...
- 若目標是最快產出 Senior Backend 履歷素材，payment Top 5 flow、`game_api coupon-redeem-credit-grant Step 5`、`game_job daily-game-data-summary Step 5` 與 `game_job third-party-record-mongo-backup Step 5` 都已收斂；`game_job coin-flow-batch-projection Step 5` 與 `game_job online-payment-data-cleaning Step 5` 也已完成但不更新正式履歷 / 自傳；下一條同 project candidate 是 `game_job partition-table-creation Step 3`。
...
| iwin | game_job | `partition-table-creation` | 每日 / 每月分表建立 | 中低 | Step 2 | 專案存在 / code-backed；Nick 貢獻依三層 claim gate 判斷 | 否 | Step 3 |
...
- `game_job coin-flow-batch-projection` 已完成 Step 5，正式履歷 / 自傳不更新。
- `game_job online-payment-data-cleaning` 已完成 Step 5；下一步回同 project ranking，做 `partition-table-creation Step 3`。
...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | iwin | game_job | `partition-table-creation` | 每日 / 每月分表建立 | 前四條 game_job flow 已完成 Step 5；下一條補 table rollover / schema rollout 可靠性 | `iwin game_job partition-table-creation Step 3` |
| 2 | iwin | iwin_gameserver | `center-http-deposit-withdraw` | center_http 上分 / 下分 | money correctness / center wallet mutation / idempotency | 待 game_job online payment flow 後再排 Step 3 |
| 3 | iwin | third_games_api | `gsc-transfer-bet-settle-rollback` | GSC transfer 投注 / 派彩 / rollback | 已完成 Step 4，下一步可做 claim gate | 待 game_job online payment flow 後再排 |

...
| 10 | ugsoft | ugsoft-connector-api | `request-bet-record-mq-sync` | UGSoft bet record job sync / late data 補資料 | 可選加強：第一條 `transfer-wallet-in-out-query` 已完成 Step 5；第二條 `provider-callback-bet-settle-to-mq` 已完成 Step 5；第三條已完成 Step 3；若 Nick 要繼續補非 iwin 廣度，可做正式面試 case | `ugsoft ugsoft-connector-api request-bet-record-mq-sync Step 4` |

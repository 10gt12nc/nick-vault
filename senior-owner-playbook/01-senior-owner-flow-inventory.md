...
- `antplay-slot-game-api` 已有 contribution consolidation、Step 1 candidate flows、Step 2 ranking，`slot-bet-settle-rollback Step 5`、`transfer-wallet-money-in-out Step 5`、`request-log-rabbitmq-async Step 5` 與 `bet-record-sharding-schema-route Step 5` 已完成；下一步依同 project ranking 做 `runtime-rtp-darkpool-player-control Step 3`。
...
| 7 | antplay | antplay-slot-game-api | `runtime-rtp-darkpool-player-control` | RTP / dark pool / player control runtime decision | slot bet、transfer wallet、request log、bet-record sharding 四條代表 flow 已 Step 5；下一步補 runtime / math contract / risk-control boundary | `antplay antplay-slot-game-api runtime-rtp-darkpool-player-control Step 3` |

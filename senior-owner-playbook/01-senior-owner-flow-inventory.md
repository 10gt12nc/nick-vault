...
- `third_games_api oneapi-wallet-bet-result Step 5` 與 `third_games_api antplay-bet-settle-rollback Step 5` 已完成；若 Nick 要回原 iwin queue，下一步是 `iwin third_games_api gsc-seamless-withdraw-deposit-cancel Step 3`。
...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | iwin | third_games_api | `gsc-seamless-withdraw-deposit-cancel` | GSC seamless withdraw / deposit / rollback / cancel | 前三條 transaction flow 已 Step 5；下一步確認 GSC 分離式 endpoint 是否仍有 production / interview 價值 | `iwin third_games_api gsc-seamless-withdraw-deposit-cancel Step 3` |

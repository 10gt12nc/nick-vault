...
| iwin | third_games_api | `oneapi-wallet-bet-result` | OneAPI / PG bet_result 投派 callback | 高 | Step 3 | 專案存在 / code-backed；下游 PGTransferInOut direct evidence 歸屬 iwin_gameserver | 否，作面試素材 | 下一步 Step 4 |
...
- `third_games_api oneapi-wallet-bet-result Step 3` 已完成；若 Nick 要回原 iwin queue，下一步是 `iwin third_games_api oneapi-wallet-bet-result Step 4`。
...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | iwin | third_games_api | `oneapi-wallet-bet-result` | OneAPI / PG bet_result 投派 callback | Step 3 已完成；可轉成 HMAC / transactionId idempotency / PGTRANSFERINOUT 面試 case | `iwin third_games_api oneapi-wallet-bet-result Step 4` |

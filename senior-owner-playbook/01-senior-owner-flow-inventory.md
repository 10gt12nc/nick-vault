...
- 若目標是最快產出 Senior Backend 履歷素材，`payment-provider-callback Step 5`、`withdrawal-auto-review-refund Step 5`、`payment-order-provider-request Step 5` 都已完成；`manual-order-review-repair Step 3` 也已完成，下一步回到同 flow 做 `manual-order-review-repair Step 4`。
...
| iwin | payment | `payment-order-provider-request` | 充值建單與 provider request | 高 | Step 5 | 部分真實開發過：Pay4z / NaNapay / BFPAY / NimTestPay provider request evidence；整體金流 owner 不誇大 | 是，保守更新 | 已收斂，轉 `manual-order-review-repair` |
| iwin | payment | `manual-order-review-repair` | 人工審核 / 補單 / 訂單修復 | 中高 | Step 3 | 專案存在 / code-backed；Nick 貢獻待確認 | 否 | `iwin payment manual-order-review-repair Step 4` |
...
- `manual-order-review-repair Step 3` 已完成，下一步同 flow 做 Step 4。
- Step 4 可補人工 repair SOP、direct status repair、`PROCESSING` unknown 與 callback 晚到的面試答法。
...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | iwin | payment | `manual-order-review-repair` | 人工審核 / 補單 / 訂單修復 | Step 3 已建立；下一步補人工補償與營運修復的面試 case | `iwin payment manual-order-review-repair Step 4` |
...
| 5 | iwin | payment | `payment-channel-config-selection` | 支付列表 / 商戶設定選擇 | runtime config / Redis projection / 玩家分層 | payment 後續候選 |

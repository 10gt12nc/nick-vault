...
| antplay | antplay-slot-game-api | `step2-flow-comparison` | 選定 slot bet / settle / rollback 作第一條代表 flow | 高 | Step 2 / 2026-05-21 | 真實開發過 + code-backed；source fetch 失敗，依本地 refs / 本地 working tree | 否，先作 Flow Track 排序；不直接更新 05 / 08 | 下一步 `slot-bet-settle-rollback Step 3` |
...
- `antplay-slot-game-api` 已有 contribution consolidation、Step 1 candidate flows 與 Step 2 ranking；第一條代表 flow 已選定 `slot-bet-settle-rollback`，下一步要做 Step 3 深掃。
...
| 7 | antplay | antplay-slot-game-api | `slot-bet-settle-rollback` | Antplay 投注 / 結算 / rollback | Step 2 已完成；這是 game-api 第一條代表 flow，先做 Step 3 深掃 money correctness / state transition / rollback | `antplay antplay-slot-game-api slot-bet-settle-rollback Step 3` |

...
- `antplay-slot-game-api` 已有 contribution consolidation、Step 1 candidate flows、Step 2 ranking，`slot-bet-settle-rollback Step 5`、`transfer-wallet-money-in-out Step 5`、`request-log-rabbitmq-async Step 5` 與 `bet-record-sharding-schema-route Step 4` 已完成；下一步依同 project ranking 做 `bet-record-sharding-schema-route Step 5`。
...
| 7 | antplay | antplay-slot-game-api | `bet-record-sharding-schema-route` | Bet record / request log / transfer transaction 分表與 schema routing | Step 4 已完成；下一步做 path-specific blame / diff 與 claim gate | `antplay antplay-slot-game-api bet-record-sharding-schema-route Step 5` |

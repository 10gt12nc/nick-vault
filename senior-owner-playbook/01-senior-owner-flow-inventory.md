...
- `antplay-slot-game-api` 已有 contribution consolidation、Step 1 candidate flows、Step 2 ranking，`slot-bet-settle-rollback Step 5`、`transfer-wallet-money-in-out Step 5` 與 `request-log-rabbitmq-async Step 5` 已完成；下一步依同 project ranking 做 `bet-record-sharding-schema-route Step 3`。
...
| 7 | antplay | antplay-slot-game-api | `bet-record-sharding-schema-route` | Bet record / request log / transfer transaction 分表與 schema routing | slot bet、transfer wallet、request log 三條代表 flow 已 Step 5；下一步補高流量資料治理 / schema route 題 | `antplay antplay-slot-game-api bet-record-sharding-schema-route Step 3` |

...
| ugsoft | ugsoft-admin-api | `contribution-claim-consolidation` | 後台 API / control plane / RabbitMQ 非同步資料處理收口 | 中高 | 已完成 / 2026-05-20 | 真實開發過 + code-backed；Nick / `10gt12nc` 有大量 direct commits | 是，保守補入後台 API / async data processing | 已收斂；`ugsoft-connector-api` 只作可選加強 |
| ugsoft | ugsoft-connector-api | `contribution-claim-consolidation` | provider connector / gateway / transfer wallet / MQ 收口 | 高 | 已完成 / rolling / 2026-05-20 | 真實開發過 + code-backed；Nick / `10gt12nc` 有大量 direct commits | 是，保守補入 provider connector / transfer wallet / MQ | 待 refresh：本批三條代表 flow 已 Step 5 |
...
| 10 | ugsoft | ugsoft-connector-api | `contribution claim consolidation refresh` | UGSoft connector 三條代表 flow 後的 project-level claim refresh | 可選加強：三條代表 flow `transfer-wallet-in-out-query`、`provider-callback-bet-settle-to-mq`、`request-bet-record-mq-sync` 均已 Step 5；若 Nick 要繼續補非 iwin 廣度，可刷新 project-level claim | `ugsoft ugsoft-connector-api contribution claim consolidation refresh` |

...
- `antplay-slot-game-api` 已有 contribution consolidation、Step 1 candidate flows、Step 2 ranking，`slot-bet-settle-rollback Step 5` 已完成，且 `transfer-wallet-money-in-out Step 4` 已完成；下一步繼續同一條 flow 做 Step 5 claim gate。
...
| 7 | antplay | antplay-slot-game-api | `transfer-wallet-money-in-out` | Transfer wallet 轉入 / 轉出 / 全轉出 | Step 4 已完成；下一步做 claim gate，判斷能否回填 project-level consolidation | `antplay antplay-slot-game-api transfer-wallet-money-in-out Step 5` |

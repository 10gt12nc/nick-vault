...
AI 必須把它視為 `KB Readiness + Next Action Automation`，自動完成以下判斷，不需要 Nick 自己知道目前有多少 flow、哪些檔案要補、哪些 career / map 還缺。Teaching notes 只作讀 flow 卡住時的補洞，不列為預設盤點主線。
...
2. 掃 `projects/` 的 domain / project / flow 狀態，至少判斷 Flow Track、Career Track、Domain Map、Decision Notes 是否有明顯缺口；Teaching Notes 只在 Nick 讀 flow 卡住或明確要求時檢查。

...
用途：糖蛙線上娛樂股份有限公司「JAVA後端組長 / Backend Team Lead (Java)」JD-specific 版本。

原則：

- 以 `08-application-autobiography-zh.md` A 版為底，加入 B 版遊戲 / provider domain 與 C 版 AI-assisted / legacy takeover。
- 對外定位為 `Senior Java Backend / Hands-on Backend Tech Lead Candidate`。
- 不寫已正式管理 3-8 人 2 年，除非 Nick 另有本人確認與具體 evidence。
- 不寫完整架構師、完整 DevOps / K8s owner、完整金流 / wallet / MQ platform owner。
...
- 參與中大型遊戲 / 博弈平台後端系統開發與維護，使用 Java / Spring Boot、MySQL、Redis、MongoDB、Kafka / RabbitMQ / Quartz 等技術處理第三方 provider 串接、payment provider 對接、遊戲錢包 / 下注結算、非同步事件、排程報表、後台控制面與既有系統維護。
...
- 參與 Slot 遊戲 API / runtime、slot job / event processing 與 provider connector / gateway 維護，處理 game init、bet / settle / rollback、transfer wallet、provider login / balance / transfer、Kafka / RabbitMQ request log / bet record、分表、schema routing、Quartz / report job 與 provider fail-fast 相關流程。
...
- 參與或協助後台 API / control plane 維護，處理 login / JWT / RBAC、商戶 / provider 白名單、玩家 / 投注 / request log / 報表查詢、風控監控、RabbitMQ request log / bet record 與 Quartz / report job；此項不包裝成完整 security platform 或 access-control platform owner。
...
- 建立 AI-assisted engineering workflow，用 Codex / Claude Code 類工具輔助 code reading、需求拆解、diff review、文件同步、測試檢查、commit 收斂與 KB 回填；AI 只作輔助，最後仍由自己判斷 production 風險、測試結果、review boundary 與交付品質。
- 參與 slot math core / 多個 math module 維護與驗證，接觸 SlotMath contract、debug bet、fixedMultiBet、currency、RTP / reel strip、buy free / free spin、jackpot / symbol 與 simulation validation；此項作為遊戲平台 domain 差異化，不包裝成完整遊戲數學模型或 RTP 策略 owner。
...
- 高交易與遊戲平台 flow：payment provider、第三方遊戲 provider gateway / connector、transfer wallet、bet / settle / rollback、request log / bet record、provider transaction、source of truth 判斷。
- 工程品質與帶人基礎：Code Review、diff review、AI-assisted review、production risk checklist、log tracing、structured log、audit log、文件化、交接與 KB 維護。
- 維運與平台理解：Docker、K3s / Kubernetes 基本概念、rollout / rollback analysis、OpenObserve / log pipeline、CI/CD 與自動化測試基本觀念。
...
我是一名以 Java 後端為主的工程師，具 5 年以上平台型後端與遊戲 / 博弈系統經驗，主要工作集中在 Java / Spring Boot API、第三方 provider 串接、payment provider 對接、遊戲錢包 / 下注結算、MQ / Kafka / RabbitMQ / Quartz、排程報表、後台控制面與既有系統維護。相較於只看單一 API 是否完成，我更習慣從 production flow 的角度理解系統：入口在哪裡、資料如何流動、狀態如何轉換、哪裡是 source of truth、哪些只是 cache / report / audit evidence，以及失敗後如何補償、追蹤與恢復。
...
我目前希望往 Senior Java Backend / Platform Backend / Hands-on Tech Lead 方向發展。我的目標不是追逐新技術名詞，而是持續累積能支撐 production 的能力：狀態一致性風險判斷、冪等、補償、Kafka / MQ、資料庫效能、系統設計、線上問題診斷、Code Review 與工程流程改善。若有機會加入重視穩定性、可維護性、AI 工具落地與團隊工程品質的後端團隊，我希望以務實、可落地的方式參與核心後端服務維護、系統演進與技術帶人。

## 自我推薦

我適合 Senior Java Backend / Hands-on Backend Tech Lead 類型職缺，尤其是需要維護線上服務穩定性、處理第三方金流 / 遊戲 provider 串接、遊戲錢包 / 下注結算、MQ / Kafka / RabbitMQ 非同步流程、Redis / MongoDB / MySQL 資料狀態、既有系統接手與 production troubleshooting 的團隊。
...
我目前不會把自己包裝成純管理職主管或完整架構師；但我能承擔 hands-on lead 類型的工作：拆解任務、檢查 production risk、參與 Code Review、整理 decision、建立 AI 工具使用邊界、協助團隊理解 production flow，並用保守、可驗證的方式推動系統可維護性與工程品質。

...
- 已完成 `iwin_gameserver center-http-deposit-withdraw Step 4`，轉成正式面試 case；仍維持 `專案存在 / code-backed`，不更新正式履歷 / 自傳，下一步做 Step 5 claim gate。
...
1. `iwin_gameserver center-http-deposit-withdraw Step 5`：Flow Track 的 project-local 下一步，對 center_http 上分 / 下分做 claim gate，判斷是否維持 interview-only 或回填 project-level claim。
...
### 1. iwin iwin_gameserver center-http-deposit-withdraw Step 5
...
7. `iwin_gameserver`：Career Track consolidation 已完成；Flow Track 下一步是 `center-http-deposit-withdraw Step 5`。

...
- 已完成 `antplay antplay-slot-game-api runtime-rtp-darkpool-player-control Step 3`；2026-05-21 已追 `/game/bet` runtime path、RTP cache、dark pool total bet / win、player control、jackpot force respin 與 `SlotMathFacade` math contract。結論是可作 game API runtime decision 面試素材初版；尚未 Step 5，不直接更新 05 / 08，不寫完整 RTP 策略 / 遊戲數學 owner。
- 已完成 `antplay antplay-slot-game-api runtime-rtp-darkpool-player-control Step 4`；2026-05-21 已補 30 秒 / 90 秒 / 3 分鐘講法、STAR、Senior / Lead 追問、可用反問與正式面試邊界。下一步 Step 5 追重要 diff / blame / claim gate；不直接更新 05 / 08。
...
2026-05-21 更新：`antplay *-math fixed-multi-bet-currency-math-core-compatibility Step 5` 已完成，`rtp-reel-strip-simulation-validation Step 5` 已完成，`buy-free-scatter-rtp3-result-contract Step 5` 已完成，`jackpot-symbol-hit-and-prize-scaling Step 5` 已完成，`special-wild-feature-state-transform Step 5` 已完成，且 `*-math contribution claim consolidation` refresh 已完成。這些都可作 `*-math` grouped 履歷 bullet 的強化 evidence；`antplay-slot-game-api slot-bet-settle-rollback Step 5`、`transfer-wallet-money-in-out Step 5`、`request-log-rabbitmq-async Step 5` 與 `bet-record-sharding-schema-route Step 5` 也已完成，`runtime-rtp-darkpool-player-control Step 4` 已完成。下一步若繼續 antplay game-api，建議做 `runtime-rtp-darkpool-player-control Step 5`。
...
13. `antplay-slot-game-api contribution claim consolidation`：已完成 rolling 收口，且已回填 `slot-bet-settle-rollback Step 5`、`transfer-wallet-money-in-out Step 5`、`request-log-rabbitmq-async Step 5` 與 `bet-record-sharding-schema-route Step 5`；`runtime-rtp-darkpool-player-control Step 4` 已作 runtime decision 正式面試素材。可保守補入 game runtime / betting-settlement / transfer wallet / async log / high-traffic table governance。下一步若繼續本 repo，應做 `runtime-rtp-darkpool-player-control Step 5`。
...
目前 antplay 線已完成 `antplay-slot-admin-api`、`antplay-slot-game-api`、`antplay-slot-game-job`、`math-core`、`*-math`、`math-workspace`、`platform-mock`、`buffer-id` rolling contribution consolidation。`antplay *-math fixed-multi-bet-currency-math-core-compatibility Step 5` 已完成，`rtp-reel-strip-simulation-validation Step 5` 已完成，`buy-free-scatter-rtp3-result-contract Step 5` 已完成，`jackpot-symbol-hit-and-prize-scaling Step 5` 已完成，`special-wild-feature-state-transform Step 5` 已完成，且 `*-math contribution claim consolidation` refresh 已完成；`slot-bet-settle-rollback Step 5`、`transfer-wallet-money-in-out Step 5`、`request-log-rabbitmq-async Step 5` 與 `bet-record-sharding-schema-route Step 5` 已完成，`runtime-rtp-darkpool-player-control Step 4` 已完成。若要補 game-api runtime / math boundary 題，下一步做 `runtime-rtp-darkpool-player-control Step 5`。若回 iwin 線，則回 `iwin iwin_gameserver center-http-deposit-withdraw Step 4`。
...
14. `antplay-slot-game-api`：contribution consolidation 已完成，Step 1 / Step 2 / `slot-bet-settle-rollback Step 5` / `transfer-wallet-money-in-out Step 5` / `request-log-rabbitmq-async Step 5` / `bet-record-sharding-schema-route Step 5` 已完成；`runtime-rtp-darkpool-player-control Step 4` 已完成。可作 AntPlay 遊戲 API runtime / 下注結算 / 轉帳錢包 / 分表 / RabbitMQ request log / high-traffic table governance / runtime decision 素材。下一步建議 `runtime-rtp-darkpool-player-control Step 5`。

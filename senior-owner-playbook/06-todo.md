...
- 2026-06-26：已建立候選第二條週排程 `company-system-study/`，包含 `README.md`、`project-index.md` 與 `topic-list.md`。定位是每週先選一個 project，再讀一小塊既有系統，把 code、git history、架構、資料流、incident / performance pain、trade-off 與現代化替代方案整理成 production thinking、troubleshooting、system design decision 與面試 takeaway。此線只作候選 automation 支撐，尚未設定排程；不全掃公司 repo、不記公司機密、不改履歷 / 自傳 / Story / Flow，也不把公司 code 分析直接變成履歷 claim。

...
1. `antplay-slot-game-job Step 2 + 1-2 條代表 flow`（Step 1 已完成）
...
13. `antplay-slot-game-job contribution claim consolidation`：已完成 rolling 收口；可保守補入履歷。Flow Track Step 1 已完成，下一步若延續本 repo，應做 Step 2，比較 proxy-user-data-report-projection、activity-accumulated-bet-voucher、big-win-notification、settle-pool-monitor-darkpool-sync、db-partition-job-report-routing 代表 flow。
...
15. `antplay-slot-game-job`：contribution consolidation 已完成，Flow Track Step 1 已完成；可作 AntPlay job / event processing、Kafka / Quartz、報表 projection、activity accumulated bet 與 big-win notification 素材。下一步建議 Step 2。

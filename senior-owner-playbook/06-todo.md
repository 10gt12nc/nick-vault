...
13. `antplay-slot-game-api contribution claim consolidation`：已完成 rolling 收口；可保守補入履歷。`Step 1` 已完成，候選 flow 是 slot bet-settle-rollback、transfer-wallet-money-in-out、request-log-rabbitmq-async、bet-record-sharding-schema-route、runtime-rtp-darkpool-player-control；下一步若繼續本 repo，應做 Flow Track Step 2。
...
14. `antplay-slot-game-api`：contribution consolidation 已完成，Step 1 已完成；可作 AntPlay 遊戲 API runtime / 下注結算 / 轉帳錢包 / 分表 / RabbitMQ request log 素材。下一步建議 Step 2。

...
1. `ugsoft-connector-api provider-callback-bet-settle-to-mq Step 4`；第一條 `transfer-wallet-in-out-query` 已完成 Step 5，第二條 callback / MQ flow 已完成 Step 3，後續可再補 1 條代表 flow
...
11. `ugsoft-connector-api`：contribution consolidation 已完成；可作 provider connector / transfer wallet / MQ 履歷素材。Step 1 / Step 2 已完成；`transfer-wallet-in-out-query Step 5` 已完成，`provider-callback-bet-settle-to-mq Step 3` 已完成，目前 connector Flow Track 下一步是同 flow Step 4。

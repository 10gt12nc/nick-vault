...
- 2026-06-26：已再補強 weekly packet 邊界：正式 references 必須附實際連結，不可只放佔位；`Technology Landscape` 增加 `When each technology is a better fit`；新增 `Knowledge Boundary`（Must / Should / Ignore）與 `One Common Misconception`；`Future Direction` 改成只有有意義時才放，且必須說明為什麼不是目前優先。

...
1. `ugsoft-connector-api transfer-wallet-in-out-query Step 5`；Step 4 已完成，後續可再補 1-2 條代表 flow
...
8. `ugsoft-admin-api contribution claim consolidation`：已完成 rolling 收口；可保守補入履歷。但 Flow Track 尚未建立 Step 1 / Step 2 / `flows/`，不得宣稱 flow 完整。若 Nick 指定 admin-api flow，應先做 `ugsoft ugsoft-admin-api Step 1`；若補非 iwin 廣度，connector 的 `transfer-wallet-in-out-query Step 4` 已完成，下一步可接同 flow Step 5。
9. `ugsoft-connector-api contribution claim consolidation`：已完成 rolling 收口；可保守補入履歷。Flow Track Step 1 / Step 2 已完成，已選 `transfer-wallet-in-out-query`、`provider-callback-bet-settle-to-mq`、`request-bet-record-mq-sync` 為本批代表 flows；`transfer-wallet-in-out-query Step 4` 已於 2026-05-27 完成，尚未 Step 5，不得宣稱全 project flow 完整。若 Nick 要繼續 UGSoft 非 iwin 廣度，下一步做 `ugsoft ugsoft-connector-api transfer-wallet-in-out-query Step 5`。
...
11. `ugsoft-connector-api`：contribution consolidation 已完成；可作 provider connector / transfer wallet / MQ 履歷素材。Step 1 / Step 2 已完成；`transfer-wallet-in-out-query Step 4` 已完成，目前 connector Flow Track 下一步是同 flow Step 5。

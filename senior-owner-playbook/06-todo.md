...
- 已完成 `ugsoft ugsoft-admin-api Step 1`；2026-05-27 已建立候選 flow 盤點，包含 `connect-bet-record-mq-ingestion`、`request-log-rabbitmq-admin-consumer`、`game-api-provider-white-ip-control-plane`、`daily-hourly-report-quartz-job`、`admin-auth-token-refresh-security`、`risk-monitor-alert-rabbitmq` 等。後續 Step 2 與第一條 `connect-bet-record-mq-ingestion Step 5` 已完成；下一步若繼續本 project 是第二條 `request-log-rabbitmq-admin-consumer Step 3`。
- 已完成 `ugsoft ugsoft-admin-api Step 2`；2026-05-27 已選本批代表 flows：`connect-bet-record-mq-ingestion`、`request-log-rabbitmq-admin-consumer`、`game-api-provider-white-ip-control-plane`。後續第一條 `connect-bet-record-mq-ingestion Step 5` 已完成；不直接跳其他 project，也不因 Step 2 更新 `05 / 08`。
...
2. `ugsoft-admin-api request-log-rabbitmq-admin-consumer Step 3`：若想補非 iwin 後台 control plane / RabbitMQ / audit ingestion 廣度，可選這條；`connect-bet-record-mq-ingestion` 已完成 Step 5，下一步回到第二條代表 flow。不是投遞前必做。
...
8. `ugsoft-admin-api contribution claim consolidation`：已完成 rolling 收口；可保守補入履歷。Flow Track Step 1 / Step 2 已完成，第一條 `connect-bet-record-mq-ingestion Step 5` 已建立，但本批其餘兩條代表 flow 尚未完成，不得宣稱 project flow 完整。若 Nick 指定 admin-api flow，下一步應做 `ugsoft ugsoft-admin-api request-log-rabbitmq-admin-consumer Step 3`；若補非 iwin 廣度，connector 本批三條代表 flow 與 contribution refresh 已收斂，下一步可選第二條 admin-api flow。

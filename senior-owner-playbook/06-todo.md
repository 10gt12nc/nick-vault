...
1. `iwin_gameserver game-spin-settlement-log-reel Step 5`：Flow Track 的 project-local 下一步；Step 4 已把 `slots-game40-sgj` spin / center wallet sync / `log_reel` 轉成正式面試 case，接著做單條 flow claim gate。
...
### 1. iwin iwin_gameserver game-spin-settlement-log-reel Step 5
...
7. `iwin_gameserver`：Career Track consolidation 已完成；`center-http-deposit-withdraw Step 5` 已完成並維持 interview-only；`game-spin-settlement-log-reel Step 4` 已完成，Flow Track 下一步是 Step 5。

...
1. `ugsoft-connector-api provider-callback-bet-settle-to-mq Step 3`；第一條 `transfer-wallet-in-out-query` 已完成 Step 5，後續可再補 1-2 條代表 flow
...
11. `ugsoft-connector-api`：contribution consolidation 已完成；可作 provider connector / transfer wallet / MQ 履歷素材。Step 1 / Step 2 已完成；`transfer-wallet-in-out-query Step 5` 已完成，目前 connector Flow Track 下一步是第二順位 `provider-callback-bet-settle-to-mq Step 3`。

...
1. `iwin_gameserver game-spin-settlement-log-reel Step 4`：Flow Track 的 project-local 下一步；Step 3 已選 `slots-game40-sgj` 作代表 game，並追到 center wallet sync / `log_reel`，接著轉正式面試 case。
...
### 1. iwin iwin_gameserver game-spin-settlement-log-reel Step 4
...
7. `iwin_gameserver`：Career Track consolidation 已完成；`center-http-deposit-withdraw Step 5` 已完成並維持 interview-only；`game-spin-settlement-log-reel Step 3` 已完成，Flow Track 下一步是 Step 4。

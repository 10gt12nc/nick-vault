...
1. `game_api agent-bonus-receive-transfer Step 4`：Step 3 已完成；下一步把代理佣金領取 / 轉帳 flow 轉成正式面試 case。
...
### 1. iwin game_api agent-bonus-receive-transfer Step 4
...
- `agent-bonus-receive-transfer` Step 3 已完成，結論是 code-backed 面試素材、不更新正式履歷。
- Step 4 要把代理佣金、Mongo money balance、Redis projection、GM 上分與 failure window 收斂成 30 秒 / 3 分鐘 / STAR / 追問稿。
- 仍不是完整 `game_api contribution claim consolidation`；本批代表 flows 尚未都完成 Step 5。

### 2. iwin 各 project 局部下一步

目前總優先是 `iwin game_api agent-bonus-receive-transfer Step 4`。以下是近期各 project 的局部下一步：

1. `game_api`：`agent-bonus-receive-transfer Step 4`，把第三順位 money-like flow 轉成面試 case。

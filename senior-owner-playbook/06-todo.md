...
- 已完成 `ugsoft ugsoft-connector-api provider-callback-bet-settle-to-mq Step 4`；2026-05-27 已轉成正式面試 case，主軸是 provider callback -> merchant callback -> MQ -> admin consumer -> bet record 的 eventual consistency；目前仍不直接更新 `05 / 08 / 04 / 17`，下一步是同 flow Step 5 claim gate。
...
1. `ugsoft-connector-api provider-callback-bet-settle-to-mq Step 5`；第一條 `transfer-wallet-in-out-query` 已完成 Step 5，第二條 callback / MQ flow 已完成 Step 4，後續可再補 1 條代表 flow
...
11. `ugsoft-connector-api`：contribution consolidation 已完成；可作 provider connector / transfer wallet / MQ 履歷素材。Step 1 / Step 2 已完成；`transfer-wallet-in-out-query Step 5` 已完成，`provider-callback-bet-settle-to-mq Step 4` 已完成，目前 connector Flow Track 下一步是同 flow Step 5。

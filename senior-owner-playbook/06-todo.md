...
- 已完成 `payment payment-order-provider-request Step 3`，建立充值建單與 provider request 主學習包；目前仍不更新正式履歷 / 自傳。
...
### 1. iwin payment payment-order-provider-request Step 4
...
- 下一步應轉成面試 case，補 provider request timeout、unknown、idempotency、query / reconciliation 追問。
- 預期仍只作 code-backed flow，不更新正式履歷，除非 Nick 補本人 evidence。

### 2. iwin 各 project 下一步

目前各 project 自己的下一步：

1. `payment`：`iwin payment payment-order-provider-request Step 4`
...
7. `app_bi`：主要 flow 已收斂；下一步不回 app_bi，payment callback 與 payment withdrawal 都已 Step 5，payment provider request 已 Step 3，轉 `iwin payment payment-order-provider-request Step 4`。
...
若 Nick 問「所有 repo 排序 / 下一個 repo」，以 `01-senior-owner-flow-inventory.md` 的「跨 repo 優先排序」為準。這份排序只用來選題，不是 code evidence；真正開工前仍要做該 repo 的 Step 1 / Step 2。目前若目標是最快補 Senior Backend 主力素材，payment callback 與 payment withdrawal 都已完成 Step 5，payment provider request 已完成 Step 3，下一條先做 `iwin payment payment-order-provider-request Step 4`。

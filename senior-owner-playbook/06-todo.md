...
- 已完成 `ugsoft ugsoft-connector-api provider-callback-bet-settle-to-mq Step 5`；2026-05-27 已完成 claim gate，可回填 project-level provider connector / callback / MQ evidence；不直接更新 `05 / 08 / 04 / 17`，不寫完整 exactly-once / outbox / reconciliation owner。若繼續 connector Flow Track，下一步可選第三順位 `request-bet-record-mq-sync Step 3`。
...
1. `ugsoft-connector-api request-bet-record-mq-sync Step 3`；第一條 `transfer-wallet-in-out-query` 已完成 Step 5，第二條 callback / MQ flow 已完成 Step 5，後續可再補 1 條 job-driven sync 代表 flow
...
11. `ugsoft-connector-api`：contribution consolidation 已完成；可作 provider connector / transfer wallet / MQ 履歷素材。Step 1 / Step 2 已完成；`transfer-wallet-in-out-query Step 5` 已完成，`provider-callback-bet-settle-to-mq Step 5` 已完成，目前 connector Flow Track 下一步可選第三順位 `request-bet-record-mq-sync Step 3`。

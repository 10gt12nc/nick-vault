...
- 2026-06-26：已記錄學歷 / 證照 / 管理進修 ROI 判斷到 `16-career-skill-matrix.md` 與 `22-career-industry-kb-evolution-plan.md`。結論：近期不把資工碩士、MBA、PMP、TOGAF 或雲端 / 架構師證照當主線；除非目標公司或實際職責需要，否則只作長期選項。2026-2027 仍優先 Senior Java Backend / Platform Backend 的 production flow、system design、distributed consistency、incident / troubleshooting、JVM / MySQL / Redis / Kafka、英文文件與面試表達；GM / MBA / PMP 等高階管理投資延後到真的承擔組織、預算、P&L 或事業單位責任時再評估。
...
- 2026-06-26：第二排程後續決策已補齊：暫不把 company code deep dive 加進 `Backend Weekly Learning`，避免 weekly learning 變成公司 repo 深掃或新壓力；`company-system-deep-dive/` 不刪、不移到 archive，只保留在原位置作 paused reference。當前分工是 `Backend Weekly Learning = active`、`company-system-deep-dive = paused reference`、`第二排程 = 不做`。
- 2026-06-26：`Backend Weekly Learning` 已小幅優化成更像 Senior 訓練、而不是文章包。新增 weekly mode（Concept / Troubleshooting / Trade-off / Interview）、Known Production Case Lens（最多 5 bullets，只用既有 notes 或泛化 production analogy，不掃公司 repo）、Mini ADR、Observability Anchor 與 Learning Check。第二排程仍不啟動；這些只是強化 active weekly packet 的 production sense、decision training、incident thinking 與 interview readiness。
- 2026-06-26：`Backend Weekly Learning` 再收斂為 `Weekly Senior Backend Capability Builder`。它不是技術快訊或文章包，而是每週用一個主題訓練 production、trade-off、incident、system design、decision making 與 interview expression。References 從最多 3 篇改為最多 2 篇：優先 1 篇官方文件，第 2 篇只有明顯 practical value 才放。新增 `Technology Landscape` 與 `Future Direction`，用 Learn Now / Learn Later / Awareness Only 管住廣度，避免因公司沒用某技術就完全沒概念，也避免把所有相關技術都變成本月待辦。
- 2026-06-26：已再補強 weekly packet 邊界：正式 references 必須附實際連結，不可只放佔位；`Technology Landscape` 增加 `When each technology is a better fit`；新增 `Knowledge Boundary`（Must / Should / Ignore，且需說明分類理由）與 `One Common Misconception`；`Future Direction` 改成只有有意義時才放，且必須說明每項是 current priority 還是 future-only topic。
- 2026-06-26：Backend Weekly Learning prompt 封版。最後只補 4 個防呆：Avoid Hallucination（documented / inferred / speculative 分清楚，不把 assumption 當 fact）、Stable References（優先官方 / 穩定來源，避免低品質 blog）、packet 過長時優先 production / decision / trade-off / interview、Known Production Case Lens 改成最多 5 bullets。暫不加入 difficulty / beginner-mid-senior，等實際跑 3-4 週後若深度不穩再評估。
- 2026-06-26：新增 `backend-weekly-writing-guideline.md`，把 weekly learning 的「怎麼寫」從 prompt / template 拆出來。後續若 Week packet 開始有模板味、連續重複 dual-write / transaction boundary / source of truth 等既有概念，優先調整 writing guideline，而不是再改 prompt / template / plan。固定分工：`backend-weekly-plan.md` 決定學什麼，`backend-weekly-template.md` 決定章節長相，`backend-weekly-writing-guideline.md` 決定每週如何承接前文並推進新 insight，`backend-learning-log.md` 累積值得保留的學習成果。
- 2026-06-26：Backend Weekly Learning v1 正式 freeze。最後只把 `Engineering Judgment` 固定為 template 的 Deep Dive 小節，要求每週至少有 1 個 accept / reject / ask-to-improve 判斷；避免重複、承接前文、新 production insight 已由 `backend-weekly-writing-guideline.md` 管理。之後不因單次觀感再改 prompt / template / guideline；只有連續 3 週出現同一問題（例如太長、Reference 無效、Technology Landscape 空泛、engineering judgment 缺席）才開 v1.1 微調。
- 2026-06-29：Week 01-04 早期試跑 / 錯誤續跑 packet 已從 `backend-learning-log.md` 清除，`backend-weekly-plan.md` 維持 `Current week: Week 01`。下次排程會用 freeze 後的 prompt / template / writing guideline 從 Week 01 重新產正式 learning packet；舊 Week 01-04 automation run / commit / memory 都不再作為已學過紀錄。
...
- 2026-06-24：已補強 Backend weekly template 與 automation prompt 邊界：weekly automation 不是只產 6 點短 checkpoint，而是依 `backend-weekly-template.md` 產一週一主題 learning packet；每週仍保持小而可行動。2026-06-26 後來源數已更新為最多 2 篇高品質來源，並包含 Beginner-to-Senior 解釋、1 個小型 code / pseudo-code 範例、1 個簡單 Mermaid 架構 / flow 圖、Senior 面試題與 30 分鐘任務。仍不得改履歷 / 自傳 / story / 正式 flow / `04 / 05 / 08 / 17 / 19`，除非 Nick 明確要求。

...
- 已完成 `ugsoft ugsoft-admin-api Step 1`；2026-05-27 已建立候選 flow 盤點，包含 `connect-bet-record-mq-ingestion`、`request-log-rabbitmq-admin-consumer`、`game-api-provider-white-ip-control-plane`、`daily-hourly-report-quartz-job`、`admin-auth-token-refresh-security`、`risk-monitor-alert-rabbitmq` 等。下一步若繼續本 project，必須做 Step 2，不得直接跳 Step 3。
- 已完成 `ugsoft ugsoft-admin-api Step 2`；2026-05-27 已選本批代表 flows：`connect-bet-record-mq-ingestion`、`request-log-rabbitmq-admin-consumer`、`game-api-provider-white-ip-control-plane`。下一步若繼續本 project，先做 `connect-bet-record-mq-ingestion Step 3`；不直接跳其他 flow，也不因 Step 2 更新 `05 / 08`。
...
2. `ugsoft-admin-api connect-bet-record-mq-ingestion Step 3`：若想補非 iwin 後台 control plane / RabbitMQ / Quartz 廣度，可選這條；Step 1 / Step 2 已完成，下一步是第一條代表 flow。不是投遞前必做。
...
8. `ugsoft-admin-api contribution claim consolidation`：已完成 rolling 收口；可保守補入履歷。Flow Track Step 1 / Step 2 已完成，`flows/` 尚未建立，不得宣稱 flow 完整。若 Nick 指定 admin-api flow，應先做 `ugsoft ugsoft-admin-api connect-bet-record-mq-ingestion Step 3`；若補非 iwin 廣度，connector 本批三條代表 flow 與 contribution refresh 已收斂，下一步可選 `ugsoft-admin-api connect-bet-record-mq-ingestion Step 3`。

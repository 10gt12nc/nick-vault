...
- 2026-06-26：已將候選第二條週排程改為 `company-discovery/`，包含啟用前資料夾、`README.md`、`project-index.md`、`topic-list.md`、`structure.md`、`discovery-template.md`、`manual-discovery-prompt.md`、`discovery-log.md` 與五個 project 子資料夾。定位不是 summary existing KB，而是每週先選一個 project，再選一個 discovery target，從 code、git history、DB / SQL、config、architecture 與既有系統裡找出 Nick 還不知道的 0-3 個高價值 discovery；找不到就回報 `No new high-value knowledge discovered`，不新增檔案。此線只作候選 automation 支撐，尚未設定排程；不全掃公司 repo、不記公司機密、不改履歷 / 自傳 / Story / Flow，也不把公司 code 分析直接變成履歷 claim。

...
- 已完成 `antplay antplay-slot-game-api slot-bet-settle-rollback Step 4`；2026-05-21 已轉成正式面試 case，補 30 秒 / 90 秒 / 3 分鐘講法、STAR、failure scenarios、Senior / Lead 追問與 owner 改善方向。Step 4 不直接更新 05 / 08；下一步做 Step 5 claim gate。
...
13. `antplay-slot-game-api contribution claim consolidation`：已完成 rolling 收口；可保守補入履歷。`Step 1` / `Step 2` / `slot-bet-settle-rollback Step 4` 已完成；下一步若繼續本 repo，應做 `slot-bet-settle-rollback Step 5`。
...
14. `antplay-slot-game-api`：contribution consolidation 已完成，Step 1 / Step 2 / `slot-bet-settle-rollback Step 4` 已完成；可作 AntPlay 遊戲 API runtime / 下注結算 / 轉帳錢包 / 分表 / RabbitMQ request log 素材。下一步建議 `slot-bet-settle-rollback Step 5`。

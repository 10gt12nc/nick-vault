...
2. `ugsoft-admin-api Step 2`：若想補非 iwin 後台 control plane / RabbitMQ / Quartz 廣度，可選這條；Step 1 已完成，Step 2 需比較候選 flows 並選本批代表 flow。不是投遞前必做。
...
8. `ugsoft-admin-api contribution claim consolidation`：已完成 rolling 收口；可保守補入履歷。Flow Track Step 1 已完成，Step 2 / `flows/` 尚未建立，不得宣稱 flow 完整。若 Nick 指定 admin-api flow，應先做 `ugsoft ugsoft-admin-api Step 2`；若補非 iwin 廣度，connector 本批三條代表 flow 與 contribution refresh 已收斂，下一步可選 `ugsoft-admin-api Step 2`。

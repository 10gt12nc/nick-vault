...
- 已完成 `antplay antplay-slot-game-job activity-accumulated-bet-voucher Step 4`；2026-05-25 已整理成正式 reward correctness 面試 case，補 30 秒 / 90 秒 / 3 分鐘說法、STAR、failure scenarios、Senior / Lead 追問與 owner 改善方向。Step 4 不直接更新 05 / 08；下一步是同 flow Step 5 claim gate。
...
13. `antplay-slot-game-job contribution claim consolidation`：已完成 rolling 收口；可保守補入履歷。Flow Track Step 1 / Step 2 已完成，`proxy-user-data-report-projection Step 5` 已完成並回填 project-level evidence，`activity-accumulated-bet-voucher Step 4` 已完成；下一步若延續本 repo，應做 `activity-accumulated-bet-voucher Step 5`。
...
15. `antplay-slot-game-job`：contribution consolidation 已完成，Flow Track Step 1 / Step 2 已完成，`proxy-user-data-report-projection Step 5` 已完成並回填 project-level report projection / Quartz summary evidence，`activity-accumulated-bet-voucher Step 4` 已完成；可作 AntPlay job / event processing、Kafka / Quartz、報表 projection、activity accumulated bet 與 big-win notification 素材。下一步建議 `activity-accumulated-bet-voucher Step 5`。

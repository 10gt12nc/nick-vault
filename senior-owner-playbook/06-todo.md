...
2. `antplay-slot-admin-api request-log-rabbitmq-admin-consumer Step 5`：若想再補非 iwin 後台 control plane / risk ops 廣度，可選這條；`Step 1 / Step 2` 與第一條代表 flow Step 4 已於 2026-05-28 完成。`ugsoft-admin-api` 與 `ugsoft-connector-api` 本批代表 flows / contribution refresh 都已收斂。不是投遞前必做。
...
11. `antplay-slot-admin-api contribution claim consolidation`：已完成 rolling 收口；可保守補入履歷。2026-05-28 Step 1 / Step 2 與第一條代表 flow `request-log-rabbitmq-admin-consumer Step 4` 已完成；後續候選包含 Game API 白名單同步、風控通知 MQ、RTP / 暗池風控監控、玩家單點控制、auth / RBAC、super agent report。若 Nick 要補 AntPlay 後台廣度，下一步是 `request-log-rabbitmq-admin-consumer Step 5`。
...
13. `antplay-slot-admin-api`：contribution consolidation 已完成，Step 1 / Step 2 與第一條代表 flow `request-log-rabbitmq-admin-consumer Step 4` 已完成；可作 AntPlay 後台 API / 商戶控制面 / 風控監控 / RabbitMQ 與 Quartz 非同步資料處理素材。`request-log-rabbitmq-admin-consumer Step 5` 是可選加強，不是預設下一步。

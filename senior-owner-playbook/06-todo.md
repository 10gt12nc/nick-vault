...
1. `iwin_gameserver bet-target-set-query Step 3`：Flow Track 的 project-local 下一步；前三條 gameserver 代表 flow 已 Step 5，下一條補 money rule / 打碼目標設定與查詢。
...
### 1. iwin iwin_gameserver bet-target-set-query Step 3
...
7. `iwin_gameserver`：Career Track consolidation 已完成；`center-http-deposit-withdraw Step 5` 與 `game-spin-settlement-log-reel Step 5` 已完成，Flow Track 下一步是 `bet-target-set-query Step 3`。

...
- 已完成 `antplay antplay-slot-game-api transfer-wallet-money-in-out Step 3`；2026-05-21 已建立 transfer wallet 轉入 / 轉出 / 全額轉出 / 查單學習包，確認簽名、防重、transaction、lookup、DB / Redis balance 與 failure window。Step 3 先作 code-backed 面試素材，不直接更新 05 / 08；下一步 Step 4。
...
13. `antplay-slot-game-api contribution claim consolidation`：已完成 rolling 收口，且已回填 `slot-bet-settle-rollback Step 5`。可保守補入履歷。`Step 1` / `Step 2` / `slot-bet-settle-rollback Step 5` / `transfer-wallet-money-in-out Step 3` 已完成；下一步若繼續本 repo，應做 `transfer-wallet-money-in-out Step 4`。
...
14. `antplay-slot-game-api`：contribution consolidation 已完成，Step 1 / Step 2 / `slot-bet-settle-rollback Step 5` / `transfer-wallet-money-in-out Step 3` 已完成；可作 AntPlay 遊戲 API runtime / 下注結算 / 轉帳錢包 / 分表 / RabbitMQ request log 素材。下一步建議 `transfer-wallet-money-in-out Step 4`。

...
- `antplay-slot-game-api/bet-record-sharding-schema-route` 已完成 Step 3，可作高流量資料治理面試素材草稿；重點是 `@UseSchema` schema route、`pt_day` / `agent_id` partition key、table creator governance 與 current create-table job disabled caveat。Step 4 前先不列為正式面試 case。

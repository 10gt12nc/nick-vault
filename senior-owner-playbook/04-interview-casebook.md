...
- `antplay-slot-game-api/request-log-rabbitmq-async` 已完成 Step 5，可作 async audit / observability 正式面試 case，也可回填 project-level claim；已追到 game-api producer、admin-api consumer 與 Nick / `10gt12nc` #774 direct commits，但不單獨寫成完整 RabbitMQ owner，且 routing key 最終格式修正屬他人 context evidence。
- `antplay-slot-game-api/bet-record-sharding-schema-route` 已完成 Step 5，可作高流量資料治理正式面試 case，也可回填 project-level claim；重點是 `@UseSchema` schema route、`pt_day` / `agent_id` partition key、table creator governance、AOP self-call、UTC day boundary、read replica lag 與 current create-table job disabled caveat。不單獨寫成完整 sharding owner。
- `antplay-slot-game-api/runtime-rtp-darkpool-player-control` 已完成 Step 5，可作 game API runtime decision 正式面試 case，也可回填 project-level claim；重點是 `GameFacade#bet` respin loop、dark pool `maxWin`、timeout refund branch、player control setData caller、jackpot force respin 與 `SlotMathFacade` math result contract。不可寫完整 RTP 策略 / 遊戲數學 / player control / jackpot owner。
- Project-level contribution consolidation 已於 2026-05-21 refresh，確認 Nick / `10gt12nc` 在 game runtime、bet / settle / rollback、transfer wallet、request log MQ、分表與 runtime 控制有大量 direct commits；slot bet、transfer wallet、request log MQ、bet-record sharding、runtime RTP / dark pool / player control 五條代表 flows 的 Step 5 都已補強 project-level evidence，但不單獨寫成完整 slot platform / wallet / RabbitMQ / sharding / RTP owner claim。
...
- Request log MQ 已深掃代表 path：`AgentApiFacade#execute / sendRequestLogMq`、`RabbitMQConfig`、`RabbitMqService`、`RequestLogMessageDto`、admin-api `RequestLogListener`、`RequestLogListenerProcessor`、`RequestLogMapper.xml`。
- Runtime decision 已深掃代表 path：`GameFacade#bet / getBetResult`、`RtpCache`、`DarkpoolService`、`PlayerControlService`、`JackpotService`、`SlotMathFacade`。
...
- `antplay-slot-game-job/settle-pool-monitor-darkpool-sync` 已完成 Step 4，可作 Kafka settlement projection / Redis DB consistency / reset sync 的 analysis-first 面試 case；未找到 Nick / `10gt12nc` direct path-specific evidence，不回填正式履歷，也不說主導 settle pool / dark pool / risk / jackpot platform。

...
- `antplay-slot-game-api/runtime-rtp-darkpool-player-control` 已完成 Step 4，可作 game API runtime decision 正式面試素材；重點是 RTP cache、dark pool total bet / win、player control、jackpot force respin 與 `SlotMathFacade` math result contract。尚未 Step 5，不回填正式履歷 claim，不寫完整 RTP 策略 / 遊戲數學 owner。

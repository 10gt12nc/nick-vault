...
- `antplay-slot-game-api/bet-record-sharding-schema-route` 已完成 Step 4，可作高流量資料治理正式面試 case；重點是 `@UseSchema` schema route、`pt_day` / `agent_id` partition key、table creator governance、AOP self-call、UTC day boundary、read replica lag 與 current create-table job disabled caveat。Step 5 前不直接升級正式履歷 claim。

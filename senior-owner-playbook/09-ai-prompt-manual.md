...
- 優先產出可投遞成果：新增大量待辦、候選 flow、知識整理、system design、skill、automation 或新目錄前，先判斷是否直接提升面試勝率、履歷品質或薪資談判能力；若否，列為可選加強或暫不建議，不得升級為必做。`nick-vault` 目前維持 v1 `Job Search Vault`，在拿到下一份工作前不新增 `career/`、`incident/`、`decision-log/`、`industry-kb` 或 skill library。
...
- 若 Nick 要求參考 `iwin-workspace`、`math-workspace`、`test001_unity`、`test001_RenPy` 或其他 workspace 的新流程，AI 必須先輸出或內化 `值得導入 / 只作參考 / 不採用 / 仍需驗證` 判斷。`/Users/nick/Git/nick/*` 只能作 personal reference / AI workflow / side project strategy，不是公司專案 evidence。只吸收入口索引、non-goals、scope、success criteria、handoff / reviewer gate、Producer / Scope gate、baseline / evidence 分層、source / KB 分離、relation map 與 generated / curated 分離等防超做方法；不得把開發型 `docs/agent-roles/`、`kb/catalog/`、per-project `CLAUDE.md`、GDD / RTP / Unity / Ren'Py / deploy / child repo 規則搬進 `nick-vault`。若 Nick 說「維護 KB / 幫優化 / 幫改 / 值得學的抄過來」，只能把結論重寫成最小 vault 規則，優先改 `00`、`09`、`06` 或 `projects/CONVENTIONS.md`，不要新增長期資料夾。
...
- Agent Workflow 在 `nick-vault` 預設只用 Lite 版：`Scope Gate -> Evidence Gate -> Claim Gate -> Review Gate`。中量維護至少走 Scope / Claim / Review；重度深掃、flow Step、contribution consolidation、履歷正式 claim 必須四個都走。`Review Gate` 內要確認本輪交付物是純回答、KB 規則、flow、career claim、閱讀順序、面試素材或 todo。只有真正涉及 source code 實作、跨 repo 改動、測試、重構或安全修正時，才參考 `math-workspace` 的完整 `Architect -> Planner -> Coder -> Reviewer`。

...
- Spring 教學要從本 flow 的 service boundary、`@Transactional`、exception、catch / rollback、self-invocation、AOP proxy 長出來。
- Kafka / MQ 教學要從本 flow 的 topic / queue、consumer group、message schema、DB write、retry、duplicate consume、offset / ack 長出來。
- Redis 教學要從本 flow 的 key、TTL、cache / projection / lock、DB consistency 長出來。
- 沒有 local code evidence 的教學只能標為 `外部通用模式 / non-local`，不能寫成專案事實。
- 共用且會重複出現的技術原則，回收進 `senior-owner-playbook/14-technical-decision-hard-skills.md`；不要每條 flow 重複寫教科書。
...
`teaching-notes.md` 只回答四件事：

1. 本 flow 哪段 code 觸發這個技術點。
2. 這個技術點在 production 會壞在哪裡。
3. Nick 面試要怎麼用白話講。
4. 要補哪個最小基本功，不延伸成完整章節。
...
`flow.md` 是唯一主報告與預設閱讀入口。`career-interview.md` 是該 flow 的保守履歷 / 面試素材。`materials/` 只放證據、技術決策、flow-specific 教學、面試稿細節與不可誇大邊界。
...
請深掃：
- /Users/nick/Git/nick/nick-vault/projects
- /Users/nick/Git/nick/nick-vault/senior-owner-playbook
- 相關 code repo
...
- projects/{domain}/{project}/flows/{flow-name}/materials/teaching-notes.md（只有本 flow 確實需要補 SQL / Spring / Kafka / Redis 等教學時才建立）
...
22. 若本 flow 需要教學補洞，只補 3-5 個和本 flow 直接相關的技術點，不寫成教科書大全
```
...
- Project contribution consolidation 可以引用 flow evidence，但主要掃 Nick / `10gt12nc` commits、branches、重要 diff、本人確認、`projects/` 與 KB 履歷素材。`archive/` 目前已清空，不再列為必要來源。
...
- `materials/teaching-notes.md`：flow-specific 技術教學。從本 flow 的 code path 長出來，只補 SQL / Spring / Kafka / Redis / transaction 等必要基本功；沒有 code evidence 的內容標成 `外部通用模式 / non-local`。

...
狀態：已對齊新版 `backend-weekly-template.md` 與 `backend-weekly-writing-guideline.md`；作為整本 weekly learning 的第一章，負責建立 local transaction boundary 心智。
...
狀態：已對齊新版 `backend-weekly-template.md` 與 `backend-weekly-writing-guideline.md`；承接 Week 01 的 local transaction boundary，不重講 dual-write 基礎，改聚焦 transaction boundary decision。

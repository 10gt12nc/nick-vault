...
- Java 17 作為目前資深 Java / Spring 日常基線；Java 21 作為加分方向。
- Java 8 / 11 legacy codebase 維護與 migration 判斷。
- Records、sealed classes、pattern matching、virtual threads 基本概念。
- Spring Boot 3 / Spring Framework 6 心智；理解 Java 17+ baseline。
...
- Eureka / Nacos / Consul。
- Micrometer Tracing / OpenTelemetry、Zipkin / Jaeger。
- Spring Cloud Stream。
- Spring Cloud Contract 加分。
...
- Contract testing、message contract、service compatibility。
...
- Spring Boot 3+ 的 tracing 心智要以 Micrometer Tracing / OpenTelemetry 為主，不再把 Spring Cloud Sleuth 當新系統主線。
- Spring Cloud 元件名不是主軸；主軸是 timeout / retry / circuit breaker / idempotency / observability / service boundary。
...
- Batch insert / update。
- N+1 query。
- Fetch strategy / batch fetching / JDBC batching 加分。
...
- Message schema / event contract。
- Request / response schema validation。
- Deprecation policy。
- Cross-team integration change review。
...
- Index rollout / rollback。
- Backward-compatible DB change。
- Online migration / expand-contract migration。
- CDC / Debezium 加分。
...
- DLQ / DLT。
- RabbitMQ publisher confirm / consumer acknowledgement。
- RabbitMQ DLX / poison message。
- Kafka topic-partition ordering。
- Kafka retry topic / DLT。
- Non-blocking retry 加分。
- At-least-once。
...
- Messaging observability：lag、retry funnel、delivery failure、DLQ replay。
...
- Idempotent Consumer。
- CDC outbox relay / Debezium outbox 加分。
...
- Canary / progressive rollout 加分。
- Graceful termination。
- JVM basic、GC、OOM、Thread dump、Heap dump。
- Connection pool。
- Graceful shutdown、Health check。
- Cloud provider baseline：load balancer、managed DB / cache / messaging、IAM、monitoring、cost awareness。
...
- DDD strategic design：bounded context、service ownership、domain language。
- Design patterns。
...
- Spring Security。
- OAuth2 / OIDC。
- JWT / opaque token resource server。
...
1. `Java 17 / Spring Boot 3 / Micrometer Tracing / OpenTelemetry`：先補現代 Spring 版本基線與 tracing 心智。
2. `API contract / schema migration / outbox`：補 OpenAPI / Spring Cloud Contract、Flyway / Liquibase、Transactional Outbox / CDC / Idempotent Consumer，讓整合與資料一致性更像資深。
3. `Security / SLO / K8s runtime`：補 OAuth2 / OIDC / JWT、SLI / SLO / runbook / postmortem、K8s probes / resources / rollout / rollback。
4. `MQ governance / replay / observability`：補 publisher confirm、consumer ack、DLQ / DLT replay、Kafka partition ordering、retry topic、MQ lag 與 poison message 處理。
5. `DDD strategic design / service ownership`：只從 provider、wallet、settlement、reporting 等既有 production flow 長出來，不抽象背名詞。

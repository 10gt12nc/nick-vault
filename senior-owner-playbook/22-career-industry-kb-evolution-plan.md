...
- 第一版只做 Senior Backend / Platform Backend 每週 learning packet；日語暫不排入主學習排程。
...
- 2026-06-26 新增候選第二條排程 `Company Discovery`，資料夾在 `company-discovery/`。它不是通用後端學習，也不是重包既有 KB，而是每週先依 `project-index.md` 選一個 project，再依 `topic-list.md` 選一個 discovery target，從 code、git history、DB / SQL、config、architecture 與既有系統裡找出 Nick 還不知道的 0-3 個高價值 discovery。找不到新發現就明確回報 `No new high-value knowledge discovered`，不產 summary packet；不得全掃公司 repo、不得記公司機密、不得把分析素材直接回填履歷。

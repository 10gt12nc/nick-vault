...
- 若目標是最快收斂 Senior Backend 履歷 / 面試素材，`iwin payment contribution claim consolidation` 已完成，不需要因新規則重做；`game_api` 目前只有 coupon 一條完整 flow，下一步先做 `iwin game_api partner-deposit-withdraw-bill Step 3` 補足 Step 2 本批代表 flows；`game_job` Top 5 代表 flows 已完成 Step 5，之後可做 `iwin game_job contribution claim consolidation`。單條 flow Step 5 只提供 evidence，不直接代表整個 project 的履歷結論。

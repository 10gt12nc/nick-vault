...
- 若目標是最快收斂 Senior Backend 履歷 / 面試素材，`iwin payment contribution claim consolidation` 已完成，不需要因新規則重做；下一步優先做已完成正向 Step 5 evidence 但尚未 project-level consolidation 的 `iwin game_api contribution claim consolidation`，之後接 `iwin game_job contribution claim consolidation`。單條 flow Step 5 只提供 evidence，不直接代表整個 project 的履歷結論。

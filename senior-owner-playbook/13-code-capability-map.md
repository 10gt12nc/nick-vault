...
- 若 Nick 要補非 iwin 廣度，`ugsoft-connector-api` Step 1 / Step 2 已完成，`transfer-wallet-in-out-query Step 5` 與 `provider-callback-bet-settle-to-mq Step 5` 已完成，`request-bet-record-mq-sync Step 3` 已完成；之後可選補 `request-bet-record-mq-sync Step 4` 作 job-driven late data / sync 面試 case，但不要把它包裝成必要 backlog。

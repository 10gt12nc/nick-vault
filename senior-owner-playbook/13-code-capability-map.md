...
- 若目標是最快收斂 Senior Backend 履歷 / 面試素材，`iwin payment contribution claim consolidation`、`iwin game_job contribution claim consolidation`、`iwin game_api contribution claim consolidation` 都已完成，不需要因新規則重做；`app_bi` 與 `bi_share` 也已做 negative consolidation，不放正式履歷主成果。下一步可先做 `iwin iwin_gameserver contribution claim consolidation`，再回 Flow Track 補 `center-http-deposit-withdraw Step 4`；單條 flow Step 5 只提供 evidence，不直接代表整個 project 的履歷結論。

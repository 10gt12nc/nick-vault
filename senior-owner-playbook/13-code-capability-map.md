...
- 若 Nick 要補非 iwin 廣度，`ugsoft-connector-api` Step 1 / Step 2 已完成，`transfer-wallet-in-out-query Step 5` 已完成，且 `provider-callback-bet-settle-to-mq Step 3` 已完成；可先把第二順位補到 Step 4 / Step 5，或之後再補 request / bet record MQ sync 代表 flow，但不要把它包裝成必要 backlog。

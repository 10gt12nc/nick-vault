...
- 若 Nick 要補非 iwin 廣度，`ugsoft-connector-api` Step 1 / Step 2 已完成，且 `transfer-wallet-in-out-query Step 5` 已完成；可補第二順位 `provider-callback-bet-settle-to-mq Step 3`，或之後再補 request / bet record MQ sync 代表 flow，但不要把它包裝成必要 backlog。

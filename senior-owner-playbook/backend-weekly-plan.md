...
- 不把尚未做過的架構寫成既成事實。
- 不把每週未完成內容累積成學習債務。
- 不固定加入 company code deep dive；每週可有一個最多 5 bullets 的 Known Production Case Lens，但只在自然適用時連到 Nick 已知 cases。若不自然，改用泛化 production analogy。不掃公司 repo、不新增第二排程、不創造 direct ownership。

## 目前進度

```text
Current cycle: Round 1 / 48 weeks
Current week: Week 03
Current topic: Self Invocation / AOP Proxy
Next topic: Week 04 Async / Thread Pool
```
...
- 若主題不能接回 production thinking、incident、decision、system design、writing 或 interview expression，先不做；不要求每個主題都硬接回履歷、自傳、Story 或特定 Flow。

防過度準備規則：

- 可以先記、先看長期主題，但 weekly 主線只放能在 30 分鐘內推進的內容。
- `Java / Spring / DB / Redis / MQ / Security / Observability / K8s 基礎` 屬於學習型能力，可以用文章、Lab、side project 補到一定程度。
- `Ownership / Decision Making / Incident Handling / Risk Communication / Business Thinking` 屬於責任型能力，只能先建立 vocabulary 與回答框架，主要靠真實工作、案例整理與市場驗證成長。
- 每週若想新增主題，先問：這是在補可學的基本功，還是在逃避投遞、QA、故事、Flow、Incident、Decision 的輸出？

2026 下半年時間比例：

- A 級 `80%`：Senior Backend、面試、Production、Incident、System Design。直接影響下一份工作。
- B 級 `15%`：Platform Backend、Observability、AI 協作。知道即可，作為加分。
- C 級 `5%`：Lead、Manager、Business、GM 能力。可以收藏，不排進本月待辦。

C 級主題包含 Hiring、1:1、Performance Review、Budget、P&L、Pricing、Org Design、Culture、Business Strategy。這些有價值，但對 2026-2027 Senior Backend 求職幫助有限；除非 JD 明確要求 Lead / Manager，否則不得搶走 A 級時間。
...
- 與面試材料關聯：若本週主題自然連到三個 Story、12 條 Flow 或 30 題核心，才做保守連結；若不自然，改連到一般 backend production / system design 場景。
- Known Production Case Lens：最多 5 bullets；若自然適用，才用既有 notes 連到已知 cases；否則使用泛化 production analogy。不掃公司 repo、不做公司系統深讀。
...
- Knowledge Boundary：用 Must Understand、Should Understand、Can Ignore For Now 收斂學習邊界，並說明每項為什麼屬於該分類。
- One Common Misconception：列 1 個最容易誤解的點，強化記憶與面試避坑。
- Future Direction：只有有意義時才放；用 Senior Backend、Platform Backend、Architect 三層標出未來可能補的能力，每層最多 1 個重點，且說明它是 current priority 還是 future-only topic，不能變成本月學習債。
...
| 能力 | 狀態 | 暫定處理 |
| --- | --- | --- |
| API Design | 值得補強 | 先列 coverage watchlist；可能在第二輪插入或融入 validation / exception / pagination 題 |
| Backend Security | 值得補強 | 先列 coverage watchlist；JWT / RBAC / signature / replay attack 只在相關週次自然補 |
| Performance Thinking | 值得補強 | 先列 coverage watchlist；CPU / memory / IO / thread / DB / cache 以 troubleshooting mode 補 |
| Java Concurrency | 可補強 | 已有 thread pool / CompletableFuture；後續視面試回饋補 JUC / blocking / back pressure / virtual thread awareness |
| Architecture Patterns | 可補強 | CQRS / Outbox / Inbox / Strangler / Hexagonal / Clean Architecture 只講適用情境，不全刷 |
| Debugging Framework | 值得補強 | 融入 Incident / Troubleshooting：impact -> layer -> proof -> fix / prevent |
| Legacy Code Reading | 可補強 | 融入 Legacy Takeover / Migration，不另開公司 code deep dive |
| DDD | Awareness | 知道即可，不做主線 |

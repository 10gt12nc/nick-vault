...
SQL、Spring、Kafka、Redis、transaction 都可以教，但目前預設先不補。先讀熟主力 production flow 和履歷 case；只有 Nick 讀某條 flow 卡住、面試追問暴露基本功缺口，或 Nick 明確要求補該 flow 教學時，才從 production flow 長出教學。

本 vault 不走「先把所有技術章節完整教完」的路線，因為那會變成無限大綱，也會稀釋 Nick 最有價值的 production case。

正確順序：

```text
先讀懂 flow
-> 真的卡住或被追問打穿時，才看這條 flow 哪裡需要 SQL / Spring / Kafka / Redis / transaction
-> 只補 3-5 個和本 flow 直接相關的技術點
-> 可面試講、可追問、可回到 code
```

兩種文件分工：

- `materials/decision-notes.md`：技術選型、trade-off、owner decision。
- `materials/teaching-notes.md`：Nick 看 flow 時需要補的最小基本功教學。

`teaching-notes.md` 不寫完整課本，只寫：

- 哪段 code 觸發這個技術點。
- production 風險是什麼。
- 面試要怎麼白話說。
- 最小要補哪個觀念。
- local code-backed / 外部通用模式 / 待確認。

禁止把「沒有 teaching-notes.md」當成已完成 flow 的缺口；也禁止對既有 49 條 flow 批量補教學。

取捨標準：

- SQL：只補本 flow 的 SQL、table、index、lock、EXPLAIN、slow query，不補完整資料庫課。
- Spring：只補本 flow 的 transaction boundary、AOP proxy、rollback、exception，不補完整 Spring 課。
- Kafka / MQ：只補本 flow 的 topic、consumer、retry、duplicate consume、ordering、offset / ack，不補完整分散式系統課。
- Redis：只補本 flow 的 key、TTL、cache / lock、DB consistency，不補完整 Redis 課。
- Idempotency / consistency：只補本 flow 的 request id、business key、status transition、retry safety，不抽象背八股。

若同一個技術點在多條 flow 重複出現，應回收到本文件的共用原則，而不是每條 flow 重複寫長篇教學。
...
### 8. Horizontal Scaling / Capacity / Infra

要會回答：

- 多開 container 能解什麼，不能解什麼。
- API 層、DB、Redis、MQ、external provider 的瓶頸怎麼分開看。
- 什麼是無狀態服務，什麼是有狀態 runtime。
- 百萬流量要先定義成 DAU、CCU、QPS 或交易筆數。
- 壓測時要看 QPS、latency、error rate，也要看資料正確性與下游瓶頸。
- 為什麼 API replica 增加後，DB / Redis / MQ 可能反而先爆。

常見比較：

- scale out API vs scale database。
- stateless API replica vs stateful game runtime。
- add consumer vs increase partition。
- Redis cache vs DB source of truth。
- capacity planning vs blind over-provisioning。
- load balancer 分流 vs transaction correctness。

...
- [backend-weekly-writing-guideline.md](backend-weekly-writing-guideline.md)：每週能力包的寫作風格規則；把每週當成同一本書的下一章，避免重複講已學過的概念，要求每週至少推進一個新的 production insight。
- [company-system-deep-dive/](company-system-deep-dive/)：候選第二條 weekly automation 的 paused reference。前期資料夾、課綱、清單、模板與手動 prompt 先保留，但目前決策是 `第二排程 = 不做`、不併入 `Backend Weekly Learning`、不全掃公司 repo。若未來重啟，方向應改成 topic-first 的 `System Capability Deep Dive`：先選通用系統能力，再用公司 code / legacy system 作案例，最後抽象成可遷移能力；不是 project inventory、不是履歷 claim map、不是待辦 backlog。

...
- [company-discovery/](company-discovery/)：候選第二條 weekly automation 的資料夾、清單、模板、手動 prompt 與啟用前資料結構。用途是每週依 `project-index.md` 先選一個專案，再依 `topic-list.md` 選一個 discovery target，從 code、git history、DB / SQL、config、architecture 與既有系統裡找出 Nick 還不知道的 0-3 個高價值 discovery；目前尚未設定排程，也不是授權全掃公司 repo。

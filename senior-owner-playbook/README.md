...
- [20-game-backend-architecture-selection.md](20-game-backend-architecture-selection.md)：遊戲後端架構選型與整合策略；用來回答 side project、商業化卡牌 / 回合制 / 放置遊戲、單體 vs 微服務、iwin / antplay / ugsoft / DevOps 如何組合成新遊戲服務器。
...
- [applications/](applications/)：只有已貼明確 JD 並完成客製時才讀；例如微達軟體 Senior Java Backend 客製包。
- [19-interview-coaching-question-bank.md](19-interview-coaching-question-bank.md)：不是投遞前必讀全文；目前是 10 個大主題 / 145 題診斷池，不是全刷。只有 Nick 想用問答降低焦慮、檢查實力或開始模擬面試時才用，每輪只問 3-5 題。
...
主軸仍然是 production flow。`14`、`15`、`16`、`17`、`18`、`19`、`20` 是輔助層：卡住或要補全局 / 硬底子 / 職涯缺口 / 談薪策略 / system design 口說 / 互動問答診斷 / 遊戲後端選型時再讀，不要拿來取代 flow。

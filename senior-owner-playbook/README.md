...
- [company-system-study/](company-system-study/)：候選第二條 weekly automation 的資料夾與清單。用途是每週讀一小塊既有系統，把 code、git history、架構、資料流與痛點整理成 production thinking、troubleshooting、trade-off 與面試可用 takeaway；目前尚未設定排程，也不是授權全掃公司 repo。

...
- [company-system-study/](company-system-study/)：候選第二條 weekly automation 的資料夾、清單、模板、手動 prompt 與啟用前資料結構。用途是每週依 `project-index.md` 先選一個專案，再依 `topic-list.md` 選一個系統切片，把 code、git history、架構、資料流與痛點整理成 production thinking、troubleshooting、trade-off 與面試可用 takeaway；目前尚未設定排程，也不是授權全掃公司 repo。

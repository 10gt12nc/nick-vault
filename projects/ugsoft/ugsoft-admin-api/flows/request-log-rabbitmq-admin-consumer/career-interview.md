...
目前完成 Step 5。這份是可練習的面試稿與單條 flow claim gate；正式是否回填 `05 / 08` 仍要等 project-level consolidation refresh 或 rolling resume package 判斷。
...
這條 flow 可作非 iwin 的 RabbitMQ / async audit / admin query 面試 case，也可作 `ugsoft-admin-api` project-level 「request log / RabbitMQ 非同步資料處理」的 supporting evidence。

可放履歷的口徑:

- 參與 UGSoft 後台 API RequestLog RabbitMQ 非同步入庫與後台查詢支援，處理 request / response audit log 的 consumer、分區、重複檢查與 DB insert。

仍不直接更新 `05 / 08`。原因是單條 flow Step 5 只能回答本 flow 的 claim 邊界，不代表整個 `ugsoft-admin-api` final consolidation。正式履歷回填應等 project contribution refresh 或 rolling resume package。

...
> 後台 control plane 改一筆白名單，不是只寫 DB，而是會改變 connector runtime access-control；因此要能講清楚 DB source of truth、Redis / in-memory cache runtime view、cache stale failure window、權限 gate 與 audit。

正式口說稿放在 [career-interview.md](career-interview.md)，追問與回答防線放在 [materials/interview.md](materials/interview.md)。

Step 4 可講的 owner decision:

1. Game API 使用 Redis set 讓 runtime filter 快速判斷，但 DB + Redis 不可能原子提交。
2. Provider latest 行為使用 RabbitMQ fanout 讓 connector reload in-memory cache，但 publish / reload 都是 eventual consistency。
3. 刪除 IP 後 cache stale 比新增 IP 後 cache stale 更危險，因為可能繼續放行不該放行的來源。

...
| `flows/` | 已建立部分代表 flow | `transfer-wallet-in-out-query` 已完成 Step 5；`provider-callback-bet-settle-to-mq` 已完成 Step 5；`request-bet-record-mq-sync` 尚未建立。 |
...
後續若繼續同 project Flow Track，第二順位已完成 Step 5；下一步回到本批代表 flow 第三順位：
...
- `projects/ugsoft/ugsoft-connector-api/README.md`: 已同步第一條 flow Step 5 完成、第二條 flow Step 5 完成，下一步是第三條 `request-bet-record-mq-sync Step 3`。

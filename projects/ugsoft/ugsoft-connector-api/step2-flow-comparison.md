...
- `projects/ugsoft/ugsoft-connector-api/README.md`: 需同步 Step 2 已完成與下一步 Step 3。
- `projects/ugsoft/README.md`: 需同步 connector Flow Track 狀態。
- `projects/source-repo-inventory.md`: 需同步 UGSoft 整理狀態。
- `projects/source-repo-flow-audit.md`: 需同步 connector 已完成 Step 2，不再標成 Step 2 最值得補。
- `senior-owner-playbook/06-todo.md`: 需同步目前待辦，下一步改為第一條 flow Step 3。
- `contribution-claim-consolidation.md`: 只同步 Flow Track 狀態與 Suggested Next；不擴張履歷 claim。

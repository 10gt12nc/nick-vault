...
- Step：Step 4 / 正式面試 case
- 狀態：Step 4 已完成；Step 5 claim gate 尚未完成。
...
- 使用方式：這份是面試口說稿與追問防線，不是正式履歷回填。正式履歷是否回填，等 Step 5 判斷。
...
- 檢查本 flow 是否可升級為單條 flow claim gate。
- 分清「可放履歷」、「可面試講」、「不可誇大」。
- 回填 `materials/claim-boundary.md` 與 `ugsoft-connector-api` consolidation，但不直接更新 `05 / 08 / 04 / 17`。

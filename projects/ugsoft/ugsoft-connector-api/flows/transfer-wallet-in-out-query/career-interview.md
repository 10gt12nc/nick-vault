...
Step 4 判斷：以上只能作「候選履歷素材」。是否寫入正式履歷仍等 Step 5 claim gate 或 project-level contribution consolidation 回填判斷。
...
Step 4 已完成面試 case。下一步若 Nick 繼續同一條 flow，應做 Step 5 claim gate：重跑 path-specific history、重要 diff、Nick / `10gt12nc` commits 與 branch evidence，判斷哪些可升級為正式履歷 claim，哪些只保留為 code-backed 面試素材。

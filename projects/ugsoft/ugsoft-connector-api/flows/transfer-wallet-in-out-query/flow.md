...
同 project 的第二順位 `provider-callback-bet-settle-to-mq` 與第三順位 `request-bet-record-mq-sync` 均已完成 Step 5，且 project-level `contribution claim consolidation refresh` 已完成；目前 connector 已收斂。

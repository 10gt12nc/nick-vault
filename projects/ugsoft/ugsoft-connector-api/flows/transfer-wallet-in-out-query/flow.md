...
同 project 的第二順位 `provider-callback-bet-settle-to-mq` 與第三順位 `request-bet-record-mq-sync` 均已完成 Step 5；若繼續 `ugsoft-connector-api`，下一步回到 project-level `contribution claim consolidation refresh`。

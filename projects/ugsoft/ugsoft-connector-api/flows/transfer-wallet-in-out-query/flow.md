...
同 project 的第二順位 `provider-callback-bet-settle-to-mq` 已完成 Step 5，第三順位 `request-bet-record-mq-sync` 已完成 Step 4；若繼續 Flow Track，下一步回到第三順位 `request-bet-record-mq-sync Step 5`。

...
| `ugsoft-connector-api` | provider connector / gateway、AntPlay / DerPlay adapter、transfer wallet、MQ | Career Track 已完成 rolling；Flow Track Step 1 / Step 2 已完成；`transfer-wallet-in-out-query` Step 3 已完成 | 可保守放 provider connector / transfer wallet / MQ 素材；不寫完整 gateway owner | `transfer-wallet-in-out-query Step 4` |
| `ugsoft-admin-web` | 後台前端 | 未開始 | 通常只作入口 | 待 Nick 指定 |
| `official-web-v3` | 官網 | 未開始 | 不當主線 | 待 Nick 指定 |
| `ugsoft-workspace` | workspace / docs / harness / runbook | contribution consolidation 已完成 / rolling | supporting evidence；不放 standalone 正式履歷主成果 | 已收斂；回 connector 第一條 flow Step 3 |
...
- 因此只能說「UGSoft 履歷 claim 已有 rolling consolidation，且 connector 第一條代表 flow 已完成 Step 3」，不能說「UGSoft 全部 flow 已完整」或「已逐條深掃所有代表 flow」。
...
- 若 Nick 之後繼續同一條 flow，下一步應做 `transfer-wallet-in-out-query Step 4`，轉成面試 case。

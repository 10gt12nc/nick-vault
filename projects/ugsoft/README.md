...
- `ugsoft-admin-api` Career Track 已完成 rolling consolidation，Flow Track 已完成 Step 1 / Step 2；本批代表 flows 為 `connect-bet-record-mq-ingestion`、`request-log-rabbitmq-admin-consumer`、`game-api-provider-white-ip-control-plane`。第一條 `connect-bet-record-mq-ingestion` 已完成 Step 4，尚未 Step 5。
...
- `ugsoft-connector-api contribution claim consolidation refresh` 已完成；`ugsoft-admin-api Step 1 / Step 2` 也已完成，且 `connect-bet-record-mq-ingestion Step 4` 已建立 flow package 與正式面試 case。若 Nick 之後繼續 UGSoft，可選 `ugsoft-admin-api connect-bet-record-mq-ingestion Step 5`，但不是投遞前必做。
- `ugsoft-admin-api Step 1` 見 `projects/ugsoft/ugsoft-admin-api/step1-candidate-flows.md`；Step 2 見 `projects/ugsoft/ugsoft-admin-api/step2-flow-comparison.md`；第一條 flow 見 `projects/ugsoft/ugsoft-admin-api/flows/connect-bet-record-mq-ingestion/flow.md`。若 Nick 之後繼續本 project，下一步是 `ugsoft-admin-api connect-bet-record-mq-ingestion Step 5`。

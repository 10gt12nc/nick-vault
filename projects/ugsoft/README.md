...
2026-05-26 re-audit：已重新掃 `/Users/nick/Git/ugsoft` 各來源 repo 的 remote refs、local HEAD、Nick / `10gt12nc` commits、主要 module / path history 與既有 KB。結論：UGSoft 仍有真正值得補的 Flow Track，但只保留收斂後兩個方向：第一順位 `ugsoft-connector-api transfer-wallet-in-out-query` 已於 2026-05-27 完成 Step 5，第二順位 `provider-callback-bet-settle-to-mq` 已完成 Step 5，第三順位 `request-bet-record-mq-sync` 已完成 Step 3；另一個 project 方向是 `ugsoft-admin-api Step 1 / Step 2`。官網、前端與 workspace 不列主待辦；workspace 只作 cross-repo reconstruction / runbook supporting evidence。
...
| `ugsoft-connector-api` | provider connector / gateway、AntPlay / DerPlay adapter、transfer wallet、MQ | Career Track 已完成 rolling；Flow Track Step 1 / Step 2 已完成；`transfer-wallet-in-out-query` Step 5 已完成；`provider-callback-bet-settle-to-mq` Step 5 已完成；`request-bet-record-mq-sync` Step 3 已完成 | 可保守放 provider connector / transfer wallet / callback / MQ 素材；不寫完整 gateway owner | 第三條代表 flow 下一步：`request-bet-record-mq-sync Step 4` |
...
- `ugsoft-connector-api` 已完成 Flow Track Step 1 / Step 2；第一條代表 flow `transfer-wallet-in-out-query` 已完成 Step 5；第二條代表 flow `provider-callback-bet-settle-to-mq` 已完成 Step 5；第三條代表 flow `request-bet-record-mq-sync` 已完成 Step 3。
- 因此只能說「UGSoft 履歷 claim 已有 rolling consolidation，且 connector 兩條代表 flow 已完成 Step 5、第三條代表 flow 已完成 Step 3」，不能說「UGSoft 全部 flow 已完整」或「已逐條深掃所有代表 flow 到 Step 5」。
...
- 若 Nick 之後繼續 `ugsoft-connector-api` Flow Track，下一步應做第三順位 `request-bet-record-mq-sync Step 4`。

...
- `antplay-slot-game-job`: contribution claim consolidation 已完成 / rolling；Flow Track Step 1 / Step 2 已完成 / 2026-05-22。Nick / `10gt12nc` 有 direct commits，可保守作 Kafka / Quartz job、代理玩家報表 projection、activity accumulated bet、big-win notification、report currency / key 修正與 db partition / job config 類履歷素材；不寫完整 Kafka event platform、完整 settle pool / risk / jackpot owner 或完整 BI / report platform owner。Step 2 選定第一條代表 flow 為 `proxy-user-data-report-projection`。

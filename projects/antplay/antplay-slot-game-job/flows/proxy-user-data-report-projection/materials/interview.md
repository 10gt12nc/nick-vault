...
本檔是 `proxy-user-data-report-projection` 的正式 Step 4 面試稿。Step 3 的 code / evidence 已可沿用；本輪把它整理成面試時可直接講、可被追問、且不誇大的版本。
...
我會說這條 flow 有 direct commit evidence，包含代理用戶數據、currency 拆分與 key 重複修正；但我不會說完整 BI platform 或 exactly-once 是我主導落地。這個 case 能證明我做過 Kafka / Quartz job 報表 projection 維護，也能從 owner 角度分析 failure window。
...
這條 flow 已經可以作為面試案例使用。下一步 Step 5 要做的是 claim gate: 回頭追 current code / blame / important diff，確認哪些說法可回填 project-level consolidation，哪些只能保留為 code-backed / interview-only。

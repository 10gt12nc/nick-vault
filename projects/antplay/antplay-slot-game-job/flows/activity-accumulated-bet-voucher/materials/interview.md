...
本檔是 `activity-accumulated-bet-voucher` 的正式 Step 4 面試稿。這條 flow 是 code-backed reward correctness case，不是 Nick 主開發 claim。
...
Step 4 已完成正式面試稿。下一步 Step 5 應追 claim gate: `BetVoucherService` 下游 idempotency / unique key、Nick merge evidence 是否能補強、以及是否回填 project-level consolidation。未完成 Step 5 前，不更新 `05 / 08`。

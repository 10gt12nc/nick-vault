...
Step 5 已補查 `BetVoucherService#addVoucher` 的下游 evidence。current repo 只看得到 `BetVoucherUtils` / `BetVoucherUtilsExt` wrapper 與外部 service 呼叫，沒有 `BetVoucherService` 實作、table schema、DB unique key 或 transaction boundary；因此不能說這條 flow 已具完整 idempotency。每次發券產生新的 UUID `refId`，在本 repo evidence 下只能保守視為 trace id，不能當防重 key。

這條 flow 最終定位是:

- 可面試講: Kafka reward projection、Redis accumulate、DB voucher count guard、Daily / Period duplicate reward risk。
- 可回填 project-level consolidation: 可以，作 `antplay-slot-game-job` activity accumulated bet / voucher supporting evidence。
- 不單獨放正式履歷 bullet: 目前不建議。
- 不更新 `05 / 08`: 本 Step 只完成 flow-level claim gate。

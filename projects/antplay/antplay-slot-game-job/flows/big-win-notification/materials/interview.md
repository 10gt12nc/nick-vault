...
本檔是 `big-win-notification` 的正式面試稿，已補 Step 5 claim gate。這條 flow 是 Nick 有 direct evidence 的 Kafka derived notification case，但不是完整 push platform claim。
...
current code 每次產生新的 UUID `_id`，如果同一 settled event 重送，可能再產生一筆通知。Step 5 補查後，`BetIdPersistence` 偏向 per-agent 最大 id 進度紀錄，不是通知去重；下游 `antplay-push` 也未見 `_id` dedupe。完整做法應該用 bet id + notification type 做 deterministic idempotency key，或讓下游用 source event key 去重。
...
current code 有 `maskPlayerName`，但 payload 仍包含 `fullPlayerName`。Step 5 補查 `antplay-push` 後，本地 code 顯示它會把 JSON 原樣送到 websocket topic；如果下游不需要完整帳號，應移除或限制使用，避免把隱私資料擴散到 notification topic。
...
| 情境 | 面試回答重點 |
| --- | --- |
| Kafka event 重送 | `_id` 每次 UUID，`BetIdPersistence` 不是通知去重，下游未見 dedupe；要補 deterministic key 或下游去重。 |
...
| fullPlayerName 擴散 | `antplay-push` 會原樣轉送 JSON，public notification 不一定需要 full account，應最小化 payload。 |
...
Step 5 已完成 claim gate。這條 flow 可作 `antplay-slot-game-job` project-level supporting evidence，也可作 Kafka derived notification 面試 case；不單獨更新 `05 / 08`。面試時要主動說清楚：`_id` 不是 deterministic key、`BetIdPersistence` 不是通知去重、下游 `antplay-push` 未見 privacy filtering，因此不能誇大成完整 push platform 或 exactly-once notification。

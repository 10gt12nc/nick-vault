...
本 flow 已完成 Step 5。後續同 project 第二順位 `transfer-wallet-money-in-out` 與第三順位 `request-log-rabbitmq-async` 也已完成 Step 5；目前下一步是 `bet-record-sharding-schema-route Step 3`，不能把本 flow 當成整個 project 已完成。

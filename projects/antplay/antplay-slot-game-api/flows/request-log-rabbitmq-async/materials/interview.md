...
本檔是 `request-log-rabbitmq-async` 的 Step 4 正式面試稿。主報告在 `flow.md`；履歷邊界看 `career-interview.md` 與 `materials/claim-boundary.md`。

Step 4 沒有重新做 Level 3 逐檔逐行；本輪依 Step 3 Level 2 code-backed evidence 轉成面試 case，並補充確認 source repo local refs 狀態。兩個來源 repo 本輪嘗試 fetch 仍因內網不可達失敗，因此仍標示未確認最新遠端、依本地 refs / 本地 working tree 保守分析。

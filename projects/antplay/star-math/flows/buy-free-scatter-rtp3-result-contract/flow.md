...
## 14. Step 4 面試 Case 結論

本 flow 已完成 Step 4。主樣本仍採 `spn-math`，因為 Nick / `10gt12nc` 在 RTP_3、buyFreeWinInfo、`HAVE_3_SCATTER_WIN`、`lastSymbols` reset 有集中 direct commits；`sph-math` 用來對照相同 buy free / `RTP_3` 模式；`sfm-math` 只作補充，確認相同 pattern 存在但未深挖。

Step 4 把這條 flow 收成可面試的 Senior Backend case：買免費局不是單一 math function，而是「定價契約 + runtime routing + result contract」三層一致性問題。
...
最適合講的 failure window:

- odds 設錯或 type 找不到，造成買免費局成本不對或直接失敗。
- `RTP_3` routing 錯，造成買免費局跑到一般輪帶。
- `lastSymbols` 沒 reset，造成 scatter / wild state 被上一輪污染。
- result JSON 與 `RoundResult` / `FreeBetResult` 語意不一致，造成前端、bet record、客服查詢或 darkpool 統計對不上。

完整 wallet mutation、bet record DB schema、前端展示與 darkpool 報表仍未深掃；因此本 Step 4 是 code-backed 面試 case，不升級成完整 wallet / settlement / darkpool owner claim。

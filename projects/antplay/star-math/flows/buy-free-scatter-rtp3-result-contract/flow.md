...
本 flow 已完成 Step 3 Level 2 深掃。主樣本採 `spn-math`，因為 Nick / `10gt12nc` 在 RTP_3、buyFreeWinInfo、`HAVE_3_SCATTER_WIN`、`lastSymbols` reset 有集中 direct commits；`sph-math` 用來對照相同 buy free / `RTP_3` 模式；`sfm-math` 只作補充，確認相同 pattern 存在但未深挖。

本輪也補讀了 `antplay-slot-game-api` 的 `GameFacade` / `GameFlowFacade`，確認 runtime caller 會依 `boughtFreeSpin` 呼叫 `freeSpinBet`，並在 result JSON / totalBet 端補 odds 與 type。完整 wallet、bet record DB schema、前端展示與 darkpool 報表仍未深掃。

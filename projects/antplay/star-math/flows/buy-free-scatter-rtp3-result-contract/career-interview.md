...
本 flow 已完成 Step 4，可作為正式面試 case。它最適合包成「slot math feature 的跨層 contract consistency」案例，而不是單純「我改了 RTP_3 輪帶」。

正式履歷 master 05 / 08 本輪仍不直接更新；這是單條 flow Step 4，後續 Step 5 才做單條 flow claim gate，project-level 履歷仍以 `*-math` contribution consolidation 為準。

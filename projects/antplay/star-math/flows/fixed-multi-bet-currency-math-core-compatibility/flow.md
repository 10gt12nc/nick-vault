...
Step 4 已完成，面試主軸收斂為「多 module math contract 相容與 money-like correctness」：

- 不是單純加欄位，而是同一筆 spin 的 `lineBet`、currency multiplier、fixedMultiBet、totalBet、jackpot、debug output、result 欄位要一致。
- `math-core` default fallback 是 rollout 安全網，不是所有 module 完整支援的證明。
- math module 不負責 wallet rollback，但它的輸出會被上游扣款 / 派彩 / 對帳依賴，所以要用 money correctness 檢查。
- 面試時可講參與維護與相容調整，不講主導完整 slot math 平台。

下一步應做 Step 5，做單條 flow claim gate，判斷這條 flow 哪些可放履歷、哪些只適合面試講、哪些還需要補 evidence。

...
| `*-math` | 多個 slot game math module | contribution consolidation 已完成 / rolling / grouped；`fixed-multi-bet-currency-math-core-compatibility` Step 5 已完成；`rtp-reel-strip-simulation-validation` Step 4 已完成 | 可保守放「多個 slot math module 維護與驗證」；強 evidence 在 `sph`、`spn`、`sfm`、`setl`、`sdt`、`slc` | `antplay *-math rtp-reel-strip-simulation-validation Step 5` |

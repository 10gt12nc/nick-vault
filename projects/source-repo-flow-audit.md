...
2026-05-26 UGSoft re-audit：已重新掃 `/Users/nick/Git/ugsoft` 下各 git repo 的 remote refs、local HEAD、Nick / `10gt12nc` commits、主要 code module、path history 與既有 `projects/ugsoft` KB。結論是：UGSoft 仍有真正值得補的 Flow Track，但範圍很收斂，不是全 repo 平均掃。第一順位 `ugsoft-connector-api transfer-wallet-in-out-query` 已完成 Step 5；第二順位 `provider-callback-bet-settle-to-mq` 已完成 Step 5；第三順位 `request-bet-record-mq-sync` 已完成 Step 4；若繼續 connector Flow Track，下一步是第三順位 `request-bet-record-mq-sync Step 5`。第二個 project 方向是 `ugsoft-admin-api Step 1 / Step 2`，用來整理後台 control plane、白名單、RabbitMQ request / bet record consumer、Quartz report job 與 risk monitor。`official-web-v3`、`ugsoft-admin-web`、`ugsoft-workspace` 不列主待辦；workspace 只作 supporting evidence。
...
1. `ugsoft-connector-api`：最值得補 Flow Track。2026-05-26 re-audit 後仍維持第一順位；Step 1 / Step 2 已完成，`transfer-wallet-in-out-query Step 5` 已於 2026-05-27 完成，`provider-callback-bet-settle-to-mq Step 5` 也已完成，`request-bet-record-mq-sync Step 4` 已完成。它有真實 direct commits、交易 / provider / callback / MQ / transfer wallet，能補非 iwin 廣度；下一步可選第三順位 `request-bet-record-mq-sync Step 5`。

...
| `antplay-push` | Nick direct commits 約 9，集中加解密 / push 相關 | 無 KB | push encryption / message delivery 可作小型 supporting case | 可選加強，不優先 |
...
| `antplay-slot-admin-api` | Nick direct commits 約 195；admin / merchant auth、白名單、風控、RabbitMQ、Quartz | 只有 contribution consolidation，沒有 Step 1 / Step 2 / flows | `request-log-rabbitmq-admin-consumer`、`risk-monitor-alert-rabbitmq`、`game-api-whitelist-sync`、`rtp-darkpool-risk-monitor`、`admin-auth-rbac-merchant-scope` | 缺 Flow Track 但值得補 |
...
| `math-core` | Nick direct commits 約 20 | contribution consolidation 已完成 | 若補 Flow Track 可做 `slot-math-core-contract-debugbet`，但 `*-math` 已有 5 條代表 flow | 可選加強，不優先 |
...
| `platform-mock` | Nick direct commits 約 6，failure injection | contribution consolidation 已完成 | provider failure testing / rollback supporting | 已收斂，除非補測試故事 |
...
| `ugsoft-connector-api` | Nick direct commits 約 217；provider connector / transfer wallet / callback / MQ / circuit breaker | 只有 contribution consolidation，沒有 Step 1 / Step 2 / flows | `provider-client-login-launch-game`、`transfer-wallet-in-out-query`、`provider-callback-bet-settle-to-mq`、`request-bet-record-mq-sync`、`provider-circuit-breaker-fast-fail`、`schema-route-partition` | 缺 Flow Track 且最值得補 |
...
| `primestar/antplay-docker-deploys` | 未見 Nick direct commits | 無 KB | docker env / deploy config | 暫不建議 |
| `primestar/antplay-web-deploy` | 未見 Nick direct commits；本機落後遠端 | 無 KB | web deploy | 暫不建議 |
...
| `primestar/kafka` | 未見 Nick direct commits | 無 KB | Kafka retention / topic / consumer lag / DLQ 可作 Platform learning-only | 可選學習，不放履歷 |
| `primestar/openobserve` | 未見 Nick direct commits | 無 KB | logs / metrics / tracing / alerting 可作 observability case | 可選 Platform 加強 |

## 建議收斂

必須記入待辦，但不代表必須全部開工：

1. `ugsoft-connector-api`：最值得補 Flow Track。它有真實 direct commits、交易 / provider / callback / MQ / transfer wallet，能補非 iwin 廣度。

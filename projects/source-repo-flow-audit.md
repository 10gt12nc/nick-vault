...
1. `ugsoft-connector-api`：最值得補 Flow Track。2026-05-26 re-audit 後仍維持第一順位；Step 1 / Step 2 已完成，`transfer-wallet-in-out-query Step 5` 已於 2026-05-27 完成，`provider-callback-bet-settle-to-mq Step 4` 也已完成。它有真實 direct commits、交易 / provider / callback / MQ / transfer wallet，能補非 iwin 廣度；下一步可做同 flow `provider-callback-bet-settle-to-mq Step 5`。

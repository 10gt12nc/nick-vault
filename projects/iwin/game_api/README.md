...
9. [flows/agent-bonus-receive-transfer/flow.md](flows/agent-bonus-receive-transfer/flow.md)：第三條代表 money-like flow，代理佣金領取 / 轉帳，已完成 Step 3。
10. [flows/agent-bonus-receive-transfer/career-interview.md](flows/agent-bonus-receive-transfer/career-interview.md)：該 flow 的 Step 3 面試素材草稿。
11. [flows/agent-bonus-receive-transfer/materials/](flows/agent-bonus-receive-transfer/materials/)：Step 3 掃描範圍、evidence、decision、interview、claim boundary。
...
| `flows/agent-bonus-receive-transfer/` | Step 3 已完成 | 已完成代理佣金領取 / 轉帳 Level 2 flow 深掃；確認 `game_api` API、Mongo `agent_money`、Redis projection、GM 上分與 `game_job` 佣金結算關聯；目前未看到 Nick / `10gt12nc` direct path evidence，不更新正式履歷 |
...
- `agent-bonus-receive-transfer` 已完成 Step 3，可作代理佣金、Mongo / Redis projection、GM command consistency 的 code-backed 面試素材；目前不作正式履歷 claim。
...
- `agent-bonus-receive-transfer` Step 3 已完成，已讀清 code path、資料狀態與 failure window。
- 下一步應把這條 flow 收斂成正式面試 case。

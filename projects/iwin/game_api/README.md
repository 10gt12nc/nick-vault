...
9. [flows/agent-bonus-receive-transfer/flow.md](flows/agent-bonus-receive-transfer/flow.md)：第三條代表 money-like flow，代理佣金領取 / 轉帳，已完成 Step 4。
10. [flows/agent-bonus-receive-transfer/career-interview.md](flows/agent-bonus-receive-transfer/career-interview.md)：該 flow 的 Step 4 code-backed 面試素材。
11. [flows/agent-bonus-receive-transfer/materials/](flows/agent-bonus-receive-transfer/materials/)：Step 4 掃描範圍、evidence、decision、interview、claim boundary。
...
| `flows/agent-bonus-receive-transfer/` | Step 4 已完成 | 已完成代理佣金領取 / 轉帳 Level 2 flow 深掃與面試收斂；確認 `game_api` API、Mongo `agent_money`、Redis projection、GM 上分與 `game_job` 佣金結算關聯；目前未看到 Nick / `10gt12nc` direct path evidence，不更新正式履歷 |
| `contribution-claim-consolidation.md` | 暫緩 | 依最新 KB，前兩條 flow Step 5 仍不能直接代表整個 `game_api` project；先補後續代表 flow |
...
- `agent-bonus-receive-transfer` 已完成 Step 4，可作代理佣金、Mongo / Redis projection、GM command consistency 的 code-backed 面試素材；目前不作正式履歷 claim。
...
- `agent-bonus-receive-transfer` Step 4 已完成，已收斂成正式面試 case。
- 下一步應做單條 flow claim gate，確認是否能升級 claim；目前仍不預設更新履歷。

...
這條 flow 我會用「每天把遊戲投注明細整理成 BI 報表」來講。

原始資料在遊戲投注 log 的日分表，例如 `log_reel_{date}`。這些明細資料量比較大，後台報表不適合每次都直接掃原始 log，所以 `game_job` 會每天跑批次，把玩家投注次數、投注額、贏分、新增玩家和留存資料整理到 `log_game_daily_record`。後台 `app_bi` 再查這張 projection 表，提供每日遊戲資料彙總和 Excel 匯出。

這條不是即時交易主流程，而是報表 projection。正常流程大概是：先刪掉指定日期與平台的舊 projection，再從 `log_reel` 分表彙總出 type=0 的玩家明細，接著由 type=0 彙總成 type=1 的 summary，最後補新增玩家和留存。Iwin 路徑還會做舊資料 backup / cleanup。

我看這條 flow 時，重點不是 SQL group by，而是資料正確性。第一，PG / Antplay 的資料日和時區窗口很容易錯，history 裡也有 GMT+8 / GMT+0 修正紀錄。第二，delete + insert 雖然方便補跑，但刪除後到寫完前有空窗，下游可能讀到半成品。第三，`new_players` 是跨日累積狀態，不像單日 projection 可以直接刪掉重算。第四，backup 後 delete 如果中途失敗，要有 rows count、完成標記或 reconciliation 才知道資料有沒有漏。

所以我會保守講成：我參與過每日遊戲資料彙總 batch / BI projection 的開發維護，能說清楚 source log、projection、summary、時區邊界、重跑與 backup / cleanup 的風險；但不會講成我主導完整 BI pipeline 或完整資料平台。

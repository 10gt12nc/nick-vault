...
用途：把 Step 3 的 code-backed flow 轉成面試可講 case。正式履歷 claim 仍留到 Step 5。
...
最後我會補一句：`10gt12nc` 的 GSC commit 有把一次查全量改成分批查詢與 batch size 調整，這是局部直接 evidence；但正式履歷要等 Step 5 確認能不能保守寫成「參與 GSC Mongo backup 分批處理」。
...
Step 4 階段不建議直接放正式履歷。可以當面試分析素材。因為 code-backed 已足夠說明我能分析 retention job 的風險，但要寫成真實開發成果，需要 Step 5 再核對 `10gt12nc` 的 GSC 分批查詢 commit、Nick 本人確認與可能的 ticket / MR。

## Q8：`10gt12nc` 的 commit 可以怎麼講？

保守講法是：「我有看到 GSC backup job 從一次查全量改成分批查詢，並調整 batch size，這是降低單次批次壓力與避免大集合操作的一種維護方向；正式履歷我會等 claim gate 確認 evidence 再寫。」

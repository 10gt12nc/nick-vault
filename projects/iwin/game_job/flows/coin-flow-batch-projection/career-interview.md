...
本文件是 `iwin game_job coin-flow-batch-projection Step 4` 的正式面試 case。
...
這句不是正式履歷 bullet；Step 5 必須再做 claim gate。
...
- Step 4 已把這條 flow 轉成正式面試 case。
- 下一步應檢查 claim boundary，確認是否能更新履歷 / 自傳；目前預期大概率不更新，除非補到 Nick 本人 evidence。

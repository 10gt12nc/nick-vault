...
證據層級：專案存在 / code-backed。Step 5 已確認這條 flow 目前沒有足夠 Nick / `10gt12nc` direct path evidence 可以寫成真實開發過；保留為 batch projection、checkpoint consistency、idempotency 的面試分析素材，不更新正式履歷 / 自傳。
...
Step 4 已把本 flow 轉成「報表 projection 不是交易 truth，但仍需要 money correctness 思維」的面試 case。Step 5 已完成 claim gate：目前不更新正式履歷 / 自傳；`game_job contribution claim consolidation` 已完成，下一步回到 `iwin_gameserver center-http-deposit-withdraw Step 4`。

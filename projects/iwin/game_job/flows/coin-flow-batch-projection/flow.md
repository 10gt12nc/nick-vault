...
Step 4 已把本 flow 轉成「報表 projection 不是交易 truth，但仍需要 money correctness 思維」的面試 case。Step 5 已完成 claim gate：目前不更新正式履歷 / 自傳；`game_job contribution claim consolidation` 已完成，下一步回到 `game_api` 未完成的代表 flow。

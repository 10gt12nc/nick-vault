...
狀態：Step 1 可沿用；前三條 flow 已完成 Step 5，第四條已完成 Step 4
...
4. `online-payment-data-cleaning`：支付 / 提現資料清洗；從 payment order 分表彙整充值、提現、首充、代理充值與經濟資料。已完成 Step 4，下一步做 Step 5 claim gate。
...
| 文件 | 狀態 | 判斷 |
| --- | --- | --- |
| `projects/iwin/game_job/README.md` | 已建立 / 已同步 | 專案入口，已同步目前下一步為 `game_job online-payment-data-cleaning Step 5` |
...
| `senior-owner-playbook/06-todo.md` | 已同步 | 目前下一步已更新為 `game_job online-payment-data-cleaning Step 5` |
...
建議：已完成 Step 5 claim gate；同 project 下一條回 candidate ranking
...
證據層級：局部真實開發過 + code-backed；Nick / `10gt12nc` 有 GSC 分批查詢與 batch size 調整 commit
...
- 已完成 Step 5 claim gate；可保守列為 GSC backup 分批查詢 / batch size 調整的局部真實開發。
...
- Step 3 已確認 MySQL `user_game_behaviour_{yyyy_m}` 是累加 upsert，Mongo `log_coin_flow_{gameId}` 是 delete+insert projection。
- Step 3 已確認 main config 目前 `coinFlowEnable=false`，實際部署啟用狀態待確認。
...
- 已完成 Step 4 面試 case / decision framing。
- 已完成 Step 5 claim gate；未見足夠 Nick / `10gt12nc` direct path evidence，正式履歷 / 自傳不更新。
...
- upstream writer 的延遲與 source of truth。
- Redis checkpoint 遺失或重設後的 production replay SOP。
- app_bi 或 BI 查詢端如何使用這些 projection。

履歷邊界：

- 可作 Senior batch / incremental projection 設計題。
- Step 5 後仍不可寫履歷，只保留為 code-backed 面試素材。

### 4. `online-payment-data-cleaning`

中文名稱：充值 / 提現資料清洗與每日經濟資料
證據層級：專案存在 / code-backed；已完成 Step 3，Nick 貢獻依三層 claim gate 判斷
...
- Step 3 已確認 Mongo `online_*` insert projection、MySQL `economic_data_day_log` delete+insert、`payment_amount*` upsert 與 `DailyEconomicDataTotalJob` 下游依賴。
...
- `payment` repo source order 寫入與 app_bi 查詢端尚未深掃。

履歷邊界：

- 目前不直接寫金流成果。
- 可作 payment reporting projection 的輔助題。
- Step 3 後仍不可寫履歷，需 Step 4 / Step 5 claim gate。
...
- `online-payment-data-cleaning` 已完成 Step 4，下一步應做 claim gate。

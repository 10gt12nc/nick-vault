...
12. [flows/partition-table-creation/career-interview.md](flows/partition-table-creation/career-interview.md)：該 flow 的保守面試素材。
13. 證據、技術決策、面試稿與 claim 邊界放 `flows/{flow-name}/materials/`。
...
| `flows/partition-table-creation/flow.md` | Step 3 | 已完成每日 / 每月分表建立主學習包；目前 code-backed，不更新履歷 |
| `flows/partition-table-creation/career-interview.md` | Step 3 | 已補 30 秒 / 3 分鐘 / STAR 初稿；下一步轉正式 Step 4 面試 case |
...
- `partition-table-creation` Step 3 已完成，已把 table rollover、SQL template、`CREATE TABLE IF NOT EXISTS`、多 channel DB partial success、schema drift 與 `player_bet_demo-mouth.sql` 檔名風險整理成主報告。

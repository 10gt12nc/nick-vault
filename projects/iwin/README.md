...
| `third_games_api` | Step 1 / Step 2；`gsc-transfer-bet-settle-rollback` 已完成 Step 5；`oneapi-wallet-bet-result` 已完成 Step 5；`antplay-bet-settle-rollback` 已完成 Step 5；`gsc-seamless-withdraw-deposit-cancel` 已完成 Step 5；rolling / scoped contribution consolidation 已完成，不新增 standalone 正式履歷主成果 | 已收斂；不新增 standalone 正式履歷主成果 |
...
| `k3s-deploy` | architecture / Step 1 / Step 2；`gameserver-phased-rollout` 已完成 Step 5 claim gate，維持 interview-only | 已收斂；不更新正式履歷 |

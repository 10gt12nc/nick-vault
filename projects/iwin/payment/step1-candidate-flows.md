...
- `projects/iwin/payment/` 目前已存在；本檔保留 Step 1 候選 flow 盤點，狀態已回補到目前第一條 flow 完成 Step 5，第二條 `withdrawal-auto-review-refund` 完成 Step 5，第三條 `payment-order-provider-request` 完成 Step 5，第四條 `manual-order-review-repair` 完成 Step 5，第五條 `payment-channel-config-selection` 完成 Step 5。
...
- `projects/iwin/payment/flows/payment-channel-config-selection/`：目前已建立，完成 Step 5；已確認 payment runtime 依 Redis projection、玩家層級、channel、device、商戶設定選擇支付 / 提現入口，已轉成面試 case，Step 5 判定不更新正式履歷 / 自傳。
...
| 文件 | 狀態 | 判斷 |
| --- | --- | --- |
| `projects/iwin/payment/README.md` | 已建立 / 已同步 | 專案入口，已同步目前 payment Top 5 flow 與 project-level contribution consolidation 都已完成，下一步回到 `iwin_gameserver center-http-deposit-withdraw Step 4` |
...
- 本檔是 `payment` Step 1 的歷史盤點；目前 project 已建立，第一條 `payment-provider-callback` flow 已完成 Step 5，第二條 `withdrawal-auto-review-refund` 已完成 Step 5，第三條 `payment-order-provider-request` 已完成 Step 5。
...
| 5 | `payment-channel-config-selection` | 中 | 中高 | 未確認到可放履歷的直接 evidence | Step 5 已完成；不更新正式履歷 |
...
- payment project-level 履歷說法已保守更新並於 2026-05-20 重新覆核；`iwin_gameserver` 是否能放履歷要等 contribution consolidation 判斷。

...
- `projects/iwin/payment/` 目前已存在；本檔保留 Step 1 候選 flow 盤點，狀態已回補到目前第一條 flow 完成 Step 5，第二條 `withdrawal-auto-review-refund` 完成 Step 5，第三條 `payment-order-provider-request` 完成 Step 4。
...
- `projects/iwin/payment/flows/payment-order-provider-request/`：目前已建立，且已完成 Step 4 面試 case；下一步做 Step 5 claim gate。
...
| 文件 | 狀態 | 判斷 |
| --- | --- | --- |
| `projects/iwin/payment/README.md` | 已建立 / 已同步 | 專案入口，已同步目前下一步為 `payment-order-provider-request` Step 5 |
...
- 本檔是 `payment` Step 1 的歷史盤點；目前 project 已建立，第一條 `payment-provider-callback` flow 已完成 Step 5，第二條 `withdrawal-auto-review-refund` 已完成 Step 5，第三條 `payment-order-provider-request` 已完成 Step 4。
...
- 下一個高價值工作是 Step 5 claim gate，判斷是否能更新正式履歷 / 自傳。

會產出什麼：

- 更新 `projects/iwin/payment/flows/payment-order-provider-request/` 的 claim boundary 與是否更新履歷 / 自傳的判定。

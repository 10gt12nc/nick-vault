...
這裡我會特別盯幾個 failure window：扣分成功但建單失敗、MQ produce 失敗只 log、provider accepted 但 no callback、重複 callback / MQ retry 是否會重複退款、下游是否用 `billNo` 去重。這些是面試時可分析的資金狀態風險；但本 flow 不能包裝成 Nick 已實作完整 wallet / reconciliation 或完整 money correctness framework。
...
- 可以用它談提款 provider flow 的狀態風險、跨系統 transaction boundary、MQ retry、失敗補償、終態 guard、人工審核 fallback。
...
- 面試可講提款 provider flow、扣分後建單 failure window、自動審核條件、provider accepted 不等於成功、失敗退款防重複、`billNo` cross-system trace 與 reconciliation 邊界。
...
- 可面試講：code-backed / 分析過。可講本 flow 的 provider 狀態風險、狀態轉移、冪等、retry、補償、人工修復或 config consistency。

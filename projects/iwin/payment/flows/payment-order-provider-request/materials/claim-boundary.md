...
- 「比較明確的 code evidence 包含 Pay4z、NaNapay、BFPAY、GoldenPay，以及 NimTestPay local / SIT testing controller。」
...
- `真實開發過`：GoldenPay provider skeleton / protocol / request spec / payment spec handling / k3s enable，依 `aed5ac3`、`27f5238`、`a00170b`、`8acd150`、`20cee78`；不可包裝成 GoldenPay production owner。
...
- 只能寫「參與」或「維護」，不要寫「主導」。
- 只能寫已掃到的代表 provider，例如 Pay4z、NaNapay、BFPAY、GoldenPay；不要寫全部 provider。
...
- 禁止把 GoldenPay 寫成 Nick 主導 production launch、完整上線驗收或量化改善成果，除非後續補 ticket / release / production evidence。

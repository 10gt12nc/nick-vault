...
`payment-order-provider-request` 已轉成 Step 4 面試 case。這條 flow 補上 provider callback 之前的前半段：本地訂單如何建立、如何組 provider request、如何處理 accepted / rejected / unknown，以及如何用 `billNo` 接 callback / 查單。

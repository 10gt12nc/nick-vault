...
- 不要說 Nick 主導完整 payment 金流。
- 不要說 Nick 對接全部 provider。
- NimTestPay 只能說 local / SIT manual testing controller，不要說 production provider 上線 owner。

## Step 5 履歷口徑

可以說：

- 我參與過 iwin payment 多個第三方金流 provider 的 request / callback / query 對接與維護。
- Pay4z 是最完整的 provider request evidence：新增 `/newPay`、callback、查單、簽章與代付 service。
- NaNapay、BFPAY、NewCashPay、NimTestPay 可作輔助 evidence，但要依各自範圍說清楚。
- 我在這類 flow 會特別看 `billNo` correlation、amount unit、sign、timeout unknown、callback idempotency 與查單補償。

不要說：

- 我主導 iwin 金流系統。
- 我設計完整 provider adapter 架構。
- 我建立完整自動 reconciliation。
- 所有 provider 都已標準化或具備 end-to-end idempotency。

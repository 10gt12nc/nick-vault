...
12. [flows/payment-channel-config-selection/flow.md](flows/payment-channel-config-selection/flow.md)：第五條 flow 主研究報告，整理支付列表 / 商戶 / 玩家層級 / 提現設定選擇與 Redis projection consistency，目前完成 Step 4。
...
| `flows/payment-channel-config-selection/` | Step 4 已完成 | Level 2 深掃支付列表 / 支付詳情 / 提現設定選擇；已確認 payment runtime 消費 app_bi Redis projection，並已轉成 config consistency / partial sync / cold-cache fallback / fail closed 面試 case |
...
- `payment-channel-config-selection` Step 4 已完成，已建立 runtime config selection 面試 case。
- 下一步應做 Step 5 claim gate，檢查 Nick path-specific evidence 與履歷 / 自傳是否需要更新。
- 以目前 evidence 預期仍不更新正式履歷；需由 Step 5 最終確認。

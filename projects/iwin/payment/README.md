...
- 可放履歷：真實開發過。已確認 Nick / `10gt12nc` 在多個 provider request / callback / query / withdraw、provider sign / response parsing、payment / withdraw order consistency 相關 commits / branches 有 evidence，可保守寫「參與第三方金流 provider 對接與維護」。
...
- 目前總 queue 已移到 `game_api partner-deposit-withdraw-bill Step 5`，因為 partner money API Step 4 已完成，下一步要做單條 flow claim gate；game_api 尚未做 project-level consolidation。

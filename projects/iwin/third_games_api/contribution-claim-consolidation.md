...
| OneAPI / Antplay provider flow | OneAPI `oneapi-wallet-bet-result` 已完成 Step 5；Antplay、GSC split endpoint 與 Redis config flow 仍是 Step 2 candidate | code-backed 面試素材；OneAPI 可面試講，但不可寫成 Nick 成果 |
...
- `oneapi-wallet-bet-result Step 5` 已完成；下一步回 Step 2 ranking 第三候選 `antplay-bet-settle-rollback Step 3`。

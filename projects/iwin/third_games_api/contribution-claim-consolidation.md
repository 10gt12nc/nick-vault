...
| OneAPI / Antplay provider flow | OneAPI `oneapi-wallet-bet-result` 已完成 Step 4；Antplay、GSC split endpoint 與 Redis config flow 仍是 Step 2 candidate | code-backed 面試素材；OneAPI 已可面試講，但未完成 Step 5，不可寫成 Nick 成果 |
...
| `oneapi-wallet-bet-result` | Step 4 已完成 | code-backed 面試素材；不更新正式履歷，Nick direct evidence 歸屬仍需保守；下一步 Step 5 做 flow-level claim gate |
...
- `oneapi-wallet-bet-result Step 4` 已完成；下一步應做 Step 5 claim gate。

...
| OneAPI / Antplay provider flow | OneAPI `oneapi-wallet-bet-result` 已完成 Step 5；Antplay `antplay-bet-settle-rollback` 已完成 Step 3；GSC split endpoint 與 Redis config flow 仍是 Step 2 candidate | code-backed 面試素材；OneAPI 可面試講；Antplay 目前為 Step 3 初版，不可寫成 Nick 成果 |
...
| `antplay-bet-settle-rollback` | Step 3 已完成 | `third_games_api` 本 repo 有 10gt12nc 測試線索，但正式開發主體不足；可做三段式 money flow / retry failure window 面試對照，不寫正式成果；下一步 Step 4 |
...
- `oneapi-wallet-bet-result Step 5` 已完成，`antplay-bet-settle-rollback Step 3` 也已完成；下一步轉成 Step 4 面試 case。

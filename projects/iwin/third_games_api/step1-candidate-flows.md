...
2. `oneapi-wallet-bet-result`：OneAPI `/wallet/bet_result`，HMAC 驗簽、transaction id 冪等、PGTRANSFERINOUT、Mongo transaction，已完成 Step 5，適合面試分析 provider idempotency 與 wallet boundary，不新增正式履歷。
3. `antplay-bet-settle-rollback`：Antplay 舊版 bet / settle / rollback 三段式流程，Step 5 已完成；Mongo step evidence、gameserver command 與 retry failure window 已轉成正式面試 case，claim gate 結論維持 interview-only。
4. `gsc-seamless-withdraw-deposit-cancel`：GSC 分離式 withdraw / deposit / rollback / cancel callback，Step 4 已完成；可分析 provider 狀態機、betId / action 去重、split endpoint 是否仍為 production 主線。
...
- `projects/iwin/third_games_api/` 目前已存在；本檔保留 Step 1 候選 flow 盤點，狀態已回補到目前第一條 flow 完成 Step 5、第二條 flow 完成 Step 3。
...
| 文件 | 狀態 | 判斷 |
| --- | --- | --- |
| `projects/iwin/third_games_api/README.md` | 已建立 / 已同步 | 專案入口，已同步目前下一步為 `gsc-seamless-withdraw-deposit-cancel Step 5` |
...
- 已轉成 HMAC / `transactionId` duplicate guard / `PGTRANSFERINOUT` / gameserver success but Mongo insert failure window 的正式面試 case。
- 高價值，但目前不可寫成 Nick 成果。

Step 5 結論：

- 維持 code-backed interview-only，不新增 `third_games_api` standalone 正式履歷成果。
- 下游 PGTransferInOut direct evidence 歸屬 `iwin_gameserver`。
...
證據層級：專案存在 / code-backed；Nick 貢獻依三層 claim gate 判斷
建議：Step 4 已完成，下一步 Step 5；適合和 transfer flow 比較 provider 狀態機
...
- split endpoint 確認存在於 `GscController`：`/api/seamless/withdraw`、`/deposit`、`/rollback`、`/cancel`。
- withdraw 成功後寫 `third_transaction_gsc` step 1；deposit 寫 step 2；rollback / cancel 寫 step 3。
- downstream gameserver 對應 `GSC_BET`、`GSC_SETTLE`、`GSC_REFUND`、`GSC_OTHER`，wallet mutation 落在 `GSCTransferInOutJob` / `PlayerData.modifyAndGetCoinGSC`。
- cancel 的 `action == wager_status` check 在 current code 被註解，且 cancel 與 rollback 共用 step 3 duplicate check，是 Step 4 要轉成面試風險的重點。
- `transfer` commit history 顯示 integrated `/api/seamless/transfer` 可能是後來整合測試 / production 主線，split endpoint 需保守標成 production 待確認。

Step 4 已完成：

- 已轉成正式面試 case，包含 30 秒、90 秒、3 分鐘說法、STAR、Senior / Lead 追問與回答。
- 正式面試主軸是 split endpoint 狀態機、adapter Mongo evidence vs gameserver wallet boundary、gameserver success but Mongo fail failure window、cancel / rollback step 3 語意，以及 `/transfer` production 主線待確認。
...
- `gsc-seamless-withdraw-deposit-cancel Step 4` 已完成；下一步做單條 flow claim gate。

...
2. `oneapi-wallet-bet-result`：OneAPI `/wallet/bet_result`，HMAC 驗簽、transaction id 冪等、PGTRANSFERINOUT、Mongo transaction，已完成 Step 4，適合面試分析 provider idempotency 與 wallet boundary。
...
| 文件 | 狀態 | 判斷 |
| --- | --- | --- |
| `projects/iwin/third_games_api/README.md` | 已建立 / 已同步 | 專案入口，已同步目前下一步為 `oneapi-wallet-bet-result Step 5` |
...
- 高價值，但目前不可寫成 Nick 成果；下一步 Step 5 做 flow-level claim gate。
...
- `oneapi-wallet-bet-result Step 4` 已完成，下一步做 Step 5 claim gate。

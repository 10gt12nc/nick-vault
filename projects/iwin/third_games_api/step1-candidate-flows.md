...
2. `oneapi-wallet-bet-result`：OneAPI `/wallet/bet_result`，HMAC 驗簽、transaction id 冪等、PGTRANSFERINOUT、Mongo transaction，已完成 Step 3，適合分析 provider idempotency。
...
| 文件 | 狀態 | 判斷 |
| --- | --- | --- |
| `projects/iwin/third_games_api/README.md` | 已建立 / 已同步 | 專案入口，已同步目前下一步為 `oneapi-wallet-bet-result Step 4` |
...
- `oneapi-wallet-bet-result Step 3` 已完成，下一步轉 Step 4 面試 case。

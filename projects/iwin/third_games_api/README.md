...
9. [flows/antplay-bet-settle-rollback/flow.md](flows/antplay-bet-settle-rollback/flow.md)：Antplay 舊版 bet / settle / rollback 三段式流程 Step 4 主報告。
10. [flows/antplay-bet-settle-rollback/career-interview.md](flows/antplay-bet-settle-rollback/career-interview.md)：該 flow 的正式面試 case。
...
| `flows/antplay-bet-settle-rollback/` | Step 4 已完成 | Antplay 舊版 bet / settle / rollback 三段式正式面試 case 已建立；保守標註為 code-backed 面試素材，不新增正式履歷 |
...
- `antplay-bet-settle-rollback` 已完成 Step 4，可用來正式面試說明三段式 bet / settle / rollback state transition、Mongo step evidence、gameserver `ANTPLAY_BET / SETTLE / REFUND` 與 retry failure window；不更新正式履歷。
...
- `antplay-bet-settle-rollback` Step 4 已完成，下一步做單條 flow claim gate；目前不更新履歷。
...
- 可面試講：code-backed / 分析過。可用 GSC transfer bet / settle / rollback、OneAPI / PG bet_result 與 Antplay bet / settle / rollback flow 說明第三方 seamless wallet callback、gameserver wallet mutation、Mongo audit、retry、idempotency 與 rollback / bet_result 語意。

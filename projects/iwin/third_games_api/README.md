...
7. [flows/oneapi-wallet-bet-result/flow.md](flows/oneapi-wallet-bet-result/flow.md)：OneAPI / PG bet_result 投派 callback Step 3 主報告。
...
| `flows/oneapi-wallet-bet-result/` | Step 3 已完成 | OneAPI / PG bet_result flow 已建立 learning package；保守標註為 code-backed 面試素材，不新增正式履歷 |
...
- `oneapi-wallet-bet-result` 已完成 Step 3，可用來理解 HMAC-SHA256、`transactionId` duplicate guard、gameserver `PGTRANSFERINOUT` 與 Mongo audit failure window；不更新正式履歷。
...
- `oneapi-wallet-bet-result` Step 3 已建立 flow package，已補 OneAPI / PG 的 HMAC、transactionId idempotency 與 `PGTRANSFERINOUT` 對照素材。
- 下一步留在同 flow 轉 Step 4 面試 case；目前不更新履歷。

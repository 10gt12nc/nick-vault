...
| `flows/bet-target-set-query/flow.md` | Step 5 已完成 | coupon 打碼入口是真實開發過 + code-backed；完整打碼 flow 為 code-backed 面試素材 |
| `flows/bet-target-set-query/career-interview.md` | Step 5 已完成 | claim gate 已收斂；不新增獨立 gameserver 打碼履歷 bullet |
...
- 可面試講：third-party transfer in/out 可用「部分真實開發過 + code-backed」語氣；center_http 上分 / 下分已完成 Step 5，仍用 code-backed / 分析過語氣；game-spin-settlement-log-reel 已完成 Step 5，一般 Game40 spin 只作面試，provider log reel 支撐既有 project claim；bet-target-set-query 已完成 Step 5，作 money rule 面試素材與 coupon supporting evidence。

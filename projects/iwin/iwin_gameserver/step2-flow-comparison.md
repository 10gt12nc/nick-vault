...
第一條 `third-party-transfer-in-out` 已由 project-level consolidation 升級為保守履歷 evidence。第二條 `center-http-deposit-withdraw` 已完成 Step 5，結論是 code-backed interview-only，不更新正式履歷 / 自傳。第三條 `game-spin-settlement-log-reel` 已完成 Step 5，一般 Game40 spin 維持面試素材，provider log reel / 投派整合支線回填既有 project claim。第四條 `bet-target-set-query` 已完成 Step 5，coupon 打碼入口是真實開發過 + code-backed，完整打碼 flow 維持 code-backed 面試素材。
...
| `step1-candidate-flows.md` | 已修正 | Step 2 已完成；本輪已同步 `center-http-deposit-withdraw` Step 4 狀態 |
| `step2-flow-comparison.md` | 可沿用 / 已回補現況 | 補齊 Step 2；目前本批四條 flow 已完成 Step 5 |
...
| 3 | `game-spin-settlement-log-reel` | 遊戲 spin / 結算 / 投注流水 | `slots-gate`、`slots-center`、`slots-games`、`slots-game40-sgj`、`slots-game-log` | 高 | 一般 Game40 spin 不升級；provider log reel 支撐既有 project claim | Step 5 已完成 |
| 4 | `bet-target-set-query` | 打碼目標設定與查詢 | `slots-center`、`slots-game-log`、`slots-common`、上游 `game_api` / `app_bi` / `payment` 待確認 | 中高 | 完整打碼系統不能擴張成 direct owner claim；app_bi 本機落後 remote，僅保守採本地 code-backed | Step 5 已完成；coupon 入口可作 supporting evidence |
...
- `bet-target-set-query` Step 5 已完成，coupon 打碼入口可作 supporting evidence，完整打碼 flow 維持 code-backed 面試素材。

...
狀態：前三條候選 flow 已完成 Step 5；`bet-target-set-query` 已完成 Step 4
...
| `step2-flow-comparison.md` | 可沿用 / 已回補現況 | 補齊 Step 2；目前前三條 flow 已完成 Step 5，Rank 4 已完成 Step 4 |
...
| 4 | `bet-target-set-query` | 打碼目標設定與查詢 | `slots-center`、`slots-game-log`、`slots-common`、上游 `game_api` / `app_bi` / `payment` 待確認 | 中高 | 上游 caller、offline player、request idempotency、commExt persistence 未追完 | Step 4 已完成，下一步 Step 5 |
...
- `bet-target-set-query` Step 4 已完成，下一步繼續同一條 flow，做 claim gate。

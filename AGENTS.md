...
- 面試題 / 複習包只能從主力 production case 長出來，不建立泛用 Java / SQL / LeetCode 300 題題庫。第一輪只圍繞 payment provider、wallet / bet-settle、Kafka / report projection 產 90 秒版、3 分鐘版、追問題庫、回答要點、誇大陷阱與 case-specific 基本功。`19-interview-coaching-question-bank.md` 可保留 14 個主題的診斷池，但實際優先練 30 題核心題；30 題核心必須兼顧 Provider / Payment 與 Java Backend 基本盤，包含 slow query、consumer lag、Spring transaction 失效。Provider Integration 是主戰場專題但不取代基本盤；Real Troubleshooting 併入 Incident 練習；Architecture Evolution 是第二輪加分，不得被包裝成全刷必做。
- `19-interview-coaching-question-bank.md` 題庫 v1.0 已定稿。除非 Nick 提供實際 JD、面試回饋、或指出題目明顯錯誤，AI 不得再主動加題、重排題庫或擴張面試題。下一步預設改成：30 題核心答案草稿、30 題錄音自講、最強 3 個 project story 練到 3 分鐘、四條核心 flow 白板講法、投遞 / 面試驗證。
